...
the extended back-naur form grammar is possible to express using the provided
functions such as \ref parseEBNF() , however it is tricky to accomplish some
goals.
...
std::shared_ptr<Statement> Parser::bad_program() {
  ...
  std::cerr << "i can no recognize token[" << ebnf.old_depth.input_tokens_index
            << "] : "
            << ebnf.input_tokens->at(ebnf.old_depth.input_tokens_index).value
            << std::endl;
  std::cerr << "on this line : " << std::endl;

  int errorLine =
      ebnf.input_tokens->at(ebnf.old_depth.input_tokens_index).lineNumber;
...
// bool EBNF_is_index_frozen = false; /** boolean used by freeze_EBNFindex() and
// unfreeze_EBNFindex() */ int EBNF_frozen_index = -1; /** last index before
// freezing the EBNF index using freeze_EBNFindex(), to be used for restoration
// using unfreeze_EBNFindex()*/

Parser::Parser(std::shared_ptr<Array<token>> gtokens, const string &filename,
               const bool insider = true) {
  if (gtokens == nullptr || gtokens->size() == 0) {
    displayError(
        filename, 0, 0,
        "Internal parser error !!!, no tokens , maybe empty source file ?");
  }
  tokens = gtokens;
  ebnf.tokens = gtokens;
  fName = filename;
  ...
  dvList = std::make_shared<
      Array<std::shared_ptr<distributingVariablesStatement>>>();
  ...
  ebnf.grammar["program"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"function-list", EBNF::EXPANSION_TOKEN, &DM14::Parser::parseFunction},
        // TODO {"global-Statement",EXPANSION_TOKEN}, add all allowed Statements
        // in the global scope !?
        {"Statement", EBNF::EXPANSION_TOKEN},
        {"unknown-list", EBNF::EXPANSION_TOKEN, &DM14::Parser::bad_program}}}};

  ebnf.grammar["unknown-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND, {{".*", EBNF::REGEX_TOKEN}}}};
  ebnf.grammar["identifier"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"[a-zA-Z]+([a-zA-Z_0-9])*", EBNF::REGEX_TOKEN}}}};
  ebnf.grammar["Statement-list"] = {
      {EBNF::GRAMMAR_TOKEN_ZERO_MORE, {{"Statement", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["Statement"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"include-Statement", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseIncludes},
        {"declaration-full-Statement", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseDeclaration},
        {"for-list", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseForloop}, /** for(from -> to : step) { Statements;
                                          } */
        //{"extern-Statement", EBNF::EXPANSION_TOKEN,
        //&DM14::Parser::parseExtern}, /** extern  { c/c++ code } endextern */
        ...
        {"if-list", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseIf}, /** if [expr] {} else if[] {} else {} */
        ...
        {"while-list", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseWhile}, /** while [ cond ] { Statements } */
        //{"case-list",EBNF::EXPANSION_TOKEN}, /** case [ID/expr] in { 1) ; 2) ;
        //*) ;}   body is like map<condition,Statements> */
        //{"addparent-Statement",EBNF::EXPANSION_TOKEN},
        {"thread-Statement", EBNF::EXPANSION_TOKEN, &DM14::Parser::parseThread},
        {"function-call-list", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseFunctionCall},
        {"return-list", EBNF::EXPANSION_TOKEN, &DM14::Parser::parseReturn},
        {"break-Statement", EBNF::EXPANSION_TOKEN, &DM14::Parser::parseBreak},
        {"continue-Statement", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseContinue},
        {"nop-Statement", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseNOPStatement},
        {"expression-Statement", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseExpressionStatement}}}};

  /// the with Statement
  ebnf.grammar["include-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"with", EBNF::KEYWORD_TOKEN},
        {"include-Statement-body", EBNF::EXPANSION_TOKEN}}}};
  ebnf.grammar["include-Statement-body"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"include-Statement-package", EBNF::EXPANSION_TOKEN},
        {"include-Statement-file", EBNF::EXPANSION_TOKEN}}}};
  ebnf.grammar["include-Statement-package"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"[a-zA-Z0-9]+", EBNF::REGEX_TOKEN},
        {"use", EBNF::KEYWORD_TOKEN},
        {"[a-zA-Z0-9]+", EBNF::REGEX_TOKEN}}}};
  ebnf.grammar["include-Statement-file"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"\"[a-zA-Z0-9]+[\\.]?[[a-zA-Z0-9]+]?\"", EBNF::REGEX_TOKEN}}}};

  /// the extern Statement
  ebnf.grammar["extern-Statement"] = {{EBNF::GRAMMAR_TOKEN_AND,
                                       {{"extern", EBNF::KEYWORD_TOKEN},
                                        {"string", EBNF::DATATYPE_TOKEN},
                                        {"endextern", EBNF::KEYWORD_TOKEN}}}};

  /// the link Statement
  ebnf.grammar["link-list"] = {{EBNF::GRAMMAR_TOKEN_OR,
                                {{"link-Statement", EBNF::EXPANSION_TOKEN},
                                 {"slink-Statement", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["link-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"link", EBNF::KEYWORD_TOKEN}, {"string", EBNF::DATATYPE_TOKEN}}}};
  ebnf.grammar["slink-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"slink", EBNF::KEYWORD_TOKEN}, {"string", EBNF::DATATYPE_TOKEN}}}};

  /// the distribute Statement
  ebnf.grammar["distribute"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"distribute", EBNF::KEYWORD_TOKEN}, {";", EBNF::TERMINAL_TOKEN}}}};

  /// the break Statement
  ebnf.grammar["break-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"break", EBNF::KEYWORD_TOKEN}, {";", EBNF::TERMINAL_TOKEN}}}};

  /// the continue Statement
  ebnf.grammar["continue-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"continue", EBNF::KEYWORD_TOKEN}, {";", EBNF::TERMINAL_TOKEN}}}};

  /// the reset Statement
  ebnf.grammar["reset-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"reset", EBNF::EXPANSION_TOKEN}, {";", EBNF::TERMINAL_TOKEN}}}};
  ebnf.grammar["reset"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"reset", EBNF::KEYWORD_TOKEN},
        {"full-expression-list", EBNF::EXPANSION_TOKEN}}},
      {EBNF::GRAMMAR_TOKEN_AND, {{"reset", EBNF::KEYWORD_TOKEN}}}};

  /// the setnode Statement
  ebnf.grammar["setnode-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"setnode-expr", EBNF::EXPANSION_TOKEN}, {";", EBNF::TERMINAL_TOKEN}}}};
  ebnf.grammar["setnode-expr"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"setnode", EBNF::KEYWORD_TOKEN},
        {"full-expression-list", EBNF::EXPANSION_TOKEN}}},
      {EBNF::GRAMMAR_TOKEN_AND, {{"setnode", EBNF::KEYWORD_TOKEN}}}};
  /// the struct Statement
  ebnf.grammar["struct-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"struct", EBNF::KEYWORD_TOKEN},
        {"[a-zA-Z]+([a-zA-Z_0-9])*", EBNF::REGEX_TOKEN},
        {"struct-body", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["struct-body"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{";", EBNF::TERMINAL_TOKEN},
        {"struct-definition", EBNF::EXPANSION_TOKEN}}}};
  ebnf.grammar["struct-definition"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"{", EBNF::TERMINAL_TOKEN},
        {"struct-declaration-list", EBNF::EXPANSION_TOKEN},
        {"}", EBNF::TERMINAL_TOKEN}}}};
  ebnf.grammar["struct-declaration-list"] = {
      {EBNF::GRAMMAR_TOKEN_ONE_MORE,
       {{"declaration-full-Statement", EBNF::EXPANSION_TOKEN}}}};

  /// the for loop Statement

  ebnf.grammar["for-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"for", EBNF::KEYWORD_TOKEN},
        {"[", EBNF::TERMINAL_TOKEN},
  ...
  ebnf.grammar["loop-expression-declarator"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"declaration-list", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseDeclaration},
        {";", EBNF::TERMINAL_TOKEN, &DM14::Parser::parseNOPStatement}}}};

  ebnf.grammar["loop-expression-condition"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"expression-Statement", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseExpressionStatement},
        {";", EBNF::TERMINAL_TOKEN, &DM14::Parser::parseNOPStatement}}}};

  ebnf.grammar["logical-expression-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"logical-expression", EBNF::EXPANSION_TOKEN},
        {";", EBNF::TERMINAL_TOKEN}}}};
  ebnf.grammar["logical-expression"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
  ...
  ebnf.grammar["logical-expression-term"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"[a-zA-Z]+([a-zA-Z_0-9])*", EBNF::REGEX_TOKEN},
        {".*", EBNF::IMMEDIATE_TOKEN}}}};

  ebnf.grammar["loop-expression-step-list"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"expression-Statement", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseExpressionStatement},
        {";", EBNF::TERMINAL_TOKEN, &DM14::Parser::parseNOPStatement}}}};
  ...
  /// the if Statement
  ebnf.grammar["if-list"] = {{EBNF::GRAMMAR_TOKEN_AND,
                              {{"if", EBNF::KEYWORD_TOKEN},
                               {"[", EBNF::TERMINAL_TOKEN},
                               {"full-expression-list", EBNF::EXPANSION_TOKEN},
                               {"]", EBNF::TERMINAL_TOKEN},
                               {"compound-Statement", EBNF::EXPANSION_TOKEN},
  ...
  ebnf.grammar["elseif-list"] = {
      {EBNF::GRAMMAR_TOKEN_ZERO_MORE,
       {{"elseif-Statement", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["elseif-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"else", EBNF::KEYWORD_TOKEN}, {"if-list", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["else-list"] = {{EBNF::GRAMMAR_TOKEN_ZERO_MORE,
                                {{"else-Statement", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["else-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"else", EBNF::KEYWORD_TOKEN},
        {"compound-Statement", EBNF::EXPANSION_TOKEN}}}};
  /// the while loop Statement
  ebnf.grammar["while-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"while", EBNF::KEYWORD_TOKEN},
        {"[", EBNF::TERMINAL_TOKEN},
        //{"logical-expression",EBNF::EXPANSION_TOKEN},
        {"full-expression-list", EBNF::EXPANSION_TOKEN},
        {"]", EBNF::TERMINAL_TOKEN},
  ...
  ebnf.grammar["declaration-specifiers-list"] = {
      {EBNF::GRAMMAR_TOKEN_ZERO_MORE,
       {{"declaration-specifiers", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["declaration-specifiers"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"global", EBNF::KEYWORD_TOKEN},
  ...
  ebnf.grammar["declaration-full-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"declaration-Statement", EBNF::EXPANSION_TOKEN},
        {";", EBNF::TERMINAL_TOKEN}}}};

  // ebnf.grammar["declaration-dataflow-specifier"] =
  // {{EBNF::GRAMMAR_TOKEN_ONLY_ONE ,{{"backprop",EBNF::KEYWORD_TOKEN}}}}; //
  // should not be ok with nodist...

  // ebnf.grammar["declaration-dist-specifiers-list"] =
  // {{EBNF::GRAMMAR_TOKEN_ONLY_ONE ,{{"channel",EBNF::KEYWORD_TOKEN}}},
  //											{EBNF::GRAMMAR_TOKEN_ONLY_ONE
  //,{{"recurrent",EBNF::KEYWORD_TOKEN}}}, 											{EBNF::GRAMMAR_TOKEN_ONLY_ONE
  //,{{"nodist",EBNF::KEYWORD_TOKEN}}}};

  // ebnf.grammar["declaration-global-specifier"] =
  // {{EBNF::GRAMMAR_TOKEN_ONLY_ONE ,{{"global",EBNF::KEYWORD_TOKEN}}}};

  ebnf.grammar["declaration-datatype-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"declaration-datatype", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["declaration-datatype"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"[a-zA-Z]+[a-zA-Z_0-9]*", EBNF::REGEX_TOKEN}}}};

  ebnf.grammar["declaration-index-list"] = {
      {EBNF::GRAMMAR_TOKEN_ONLY_ONE,
       {{"declaration-index", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["declaration-index"] = {{EBNF::GRAMMAR_TOKEN_OR,
                                        {{"matrix-index", EBNF::EXPANSION_TOKEN,
                                          &DM14::Parser::parseMatrixIndex},
                                         {"array-index", EBNF::EXPANSION_TOKEN,
                                          &DM14::Parser::parseArrayIndex}}}};

  ebnf.grammar["matrix-index"] = {{EBNF::GRAMMAR_TOKEN_AND,
                                   {{"array-index", EBNF::EXPANSION_TOKEN},
                                    {"array-index", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["array-index"] = {{EBNF::GRAMMAR_TOKEN_AND,
                                  {{"[", EBNF::TERMINAL_TOKEN},
  ...
  ebnf.grammar["declaration-value-list"] = {
      {EBNF::GRAMMAR_TOKEN_ONLY_ONE,
       {{"declaration-value", EBNF::EXPANSION_TOKEN}}}};
  ebnf.grammar["declaration-value"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"=", EBNF::TERMINAL_TOKEN},
  ...
  ebnf.grammar["function-call-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"function-call", EBNF::EXPANSION_TOKEN},
        {";", EBNF::TERMINAL_TOKEN}}}};

  // ebnf.grammar["function-call"] =
  // {{EBNF::GRAMMAR_TOKEN_AND,{{"[a-zA-Z]+([a-zA-Z_0-9])*",EBNF::REGEX_TOKEN},
  ebnf.grammar["function-call"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"identifier", EBNF::EXPANSION_TOKEN},
        {"(", EBNF::TERMINAL_TOKEN},
        {"function-call-arguments-list", EBNF::EXPANSION_TOKEN},
        {")", EBNF::TERMINAL_TOKEN}}}};

  ebnf.grammar["function-call-arguments-list"] = {
      {EBNF::GRAMMAR_TOKEN_ZERO_MORE,
       {{"function-call-arguments", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["function-call-arguments"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"full-expression-list", EBNF::EXPANSION_TOKEN},
        {",", EBNF::TERMINAL_TOKEN}}},
      {EBNF::GRAMMAR_TOKEN_AND,
  ...
  ebnf.grammar["expression-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"expression-list", EBNF::EXPANSION_TOKEN},
        {";", EBNF::TERMINAL_TOKEN, &DM14::Parser::parseExpressionStatement}}}};

  ebnf.grammar["expression-list"] = {
      {EBNF::GRAMMAR_TOKEN_ONE_MORE,
       {{"full-expression-list", EBNF::EXPANSION_TOKEN,
         &DM14::Parser::parseExpressionStatement}}}};

  ebnf.grammar["full-expression-list"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
  ...
  ebnf.grammar["big-expression-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"expression-types", EBNF::EXPANSION_TOKEN},
        {"BIN_OP", EBNF::BINARY_OP_TOKEN},
        {"expression-types", EBNF::EXPANSION_TOKEN}}}};
  ebnf.grammar["expression-types"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"big-expression", EBNF::EXPANSION_TOKEN},
        {"expression", EBNF::EXPANSION_TOKEN}}}};
  ebnf.grammar["big-expression"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"(", EBNF::OP_TOKEN},
        {"big-expression-list", EBNF::EXPANSION_TOKEN},
        {")", EBNF::OP_TOKEN}}}};
  ...
  ebnf.grammar["expression-extend"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{"function-call", EBNF::EXPANSION_TOKEN},
        {"variable", EBNF::EXPANSION_TOKEN},
  ...
  ebnf.grammar["thread-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"thread-list", EBNF::EXPANSION_TOKEN}, {";", EBNF::TERMINAL_TOKEN}}}};

  ebnf.grammar["thread-list"] = {{EBNF::GRAMMAR_TOKEN_AND,
                                  {{"thread", EBNF::KEYWORD_TOKEN},
                                   {"function-call", EBNF::EXPANSION_TOKEN}}}};

  /// the function Statement
  ebnf.grammar["function-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"function-prototype", EBNF::EXPANSION_TOKEN},
        {"function-definition-list", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["function-definition-list"] = {
      {EBNF::GRAMMAR_TOKEN_OR,
       {{";", EBNF::TERMINAL_TOKEN},
        {"compound-Statement", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["function-prototype"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"identifier", EBNF::EXPANSION_TOKEN},
        {"(", EBNF::TERMINAL_TOKEN},
  ...
  ebnf.grammar["function-parameter-list"] = {
      {EBNF::GRAMMAR_TOKEN_ZERO_MORE,
       {{"function-parameters", EBNF::EXPANSION_TOKEN}}}};
  ebnf.grammar["function-parameters"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"function-parameter", EBNF::EXPANSION_TOKEN}}},
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"function-extra-parameter", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["function-extra-parameter"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{",", EBNF::TERMINAL_TOKEN},
        {"function-parameter", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["function-parameter"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"identifier", EBNF::EXPANSION_TOKEN},
        {"declaration-datatype-list", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["function-return"] = {
      {EBNF::GRAMMAR_TOKEN_ZERO_MORE,
       {{"function-parameter", EBNF::EXPANSION_TOKEN}}}};

  ebnf.grammar["compound-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"{", EBNF::TERMINAL_TOKEN},
  ...
  ebnf.grammar["return-list"] = {
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"return", EBNF::KEYWORD_TOKEN},
        {"expression-Statement", EBNF::EXPANSION_TOKEN}}},
      {EBNF::GRAMMAR_TOKEN_AND,
       {{"return", EBNF::KEYWORD_TOKEN}, {";", EBNF::TERMINAL_TOKEN}}}};
  /// nop Statement
  ebnf.grammar["nop-Statement"] = {
      {EBNF::GRAMMAR_TOKEN_AND, {{";", EBNF::TERMINAL_TOKEN}}}};

  ebnf.groupBy({"program",
                "Statement",
                "include-Statement",
                "include-Statement-body",
                "function-list",
                "function-prototype",
...
int Parser::parse() {
  if (!insider) {
    parseIncludesInsider("core/DM14GLOBAL.m14", "",
                         includePath::sourceFileType::FILE_DM14);
  }
  ...
  mapcode mapCode(fName, getFunctions(), getIncludes(), false,
                  distributedNodesCount, distributedVariablesCount);
...
std::shared_ptr<Statement> Parser::parseIncludes() {
  ...
  string package = "";
  string library = "";
  includePath::sourceFileType includeType =
      includePath::sourceFileType::LIBRARY;
  if (activeStack().getToken().type != "string" &&
      activeStack().getToken().type != "identifier" &&
      !DM14::types::isDataType(
          activeStack().getToken().value)) // tokens->at(activeStack().getIndex()).type
                                           // != "datatype")
  {
    displayError(fName, activeStack().getToken().lineNumber,
                 activeStack().getToken().columnNumber,
                 "Expected Package name and not " +
                     activeStack().getToken().value);
  }

  if (activeStack().getToken().type == "string") {

    package = activeStack().getToken().value.substr(
        1, (activeStack().getToken().value.size() - 2));
    if (package.substr(package.size() - 5) == ".m14") {
      includeType = includePath::sourceFileType::FILE_DM14;
    } else if (package.substr(package.size() - 5) == ".cpp" ||
               package.substr(package.size() - 4) == ".cc") {
...
int Parser::parseIncludesInsider(
    const string &package, const string &library,
  ...
  if (includeType == includePath::sourceFileType::FILE_DM14 ||
      includeType == includePath::sourceFileType::FILE_C) {
    string fullPath;
    ...
    for (uint32_t i = 0; i < parser.getMapCodes()->size(); i++) {
      ...
      /*for(unsigned k =0; k <
      parser.getMapCodes()->at(i).globalDefinitions.size(); k++)
      {
...
std::shared_ptr<Statement> Parser::parseLink() {
  ...
  if (activeStack().getToken().value.size() > 2) {
    stmt->libs = activeStack().getToken().value.substr(
        1, activeStack().getToken().value.size() - 2);
  }
...
std::shared_ptr<Statement> Parser::parseReturn() {
  ...
  if (!peekToken(";")) {
    // returnStatement->retValue = parseOpStatement(0, reachToken(";", false,
    // true, true, true, false)-1, currentFunction.returnIDType, 0,
    // returnStatement);
...
std::shared_ptr<Statement> Parser::parseMatrixIndex() {
  //@TODO: need to handle 2nd or multiple diemnsions , have to implement in the
  //AST first then parseDeclraration and ParseOpStatement...
  activeStack().popToken();
  RequireValue("[", "Expected [ and not : ", true);
  int from = 0;
  int to = reachToken("]", false, true, false, true, true) - 1;
  std::shared_ptr<Statement> result =
      parseOpStatement(from, to, "-2", 0, currentStatement);
...
std::shared_ptr<Statement> Parser::parseAddParent() {
  ...
  if ((tokens->at(activeStack().getIndex())).value == "(") {
    ...
    ps->ip =
        parseOpStatement(cindex, reachToken(")", false, true, true, true, true),
                         "string", 0, ps);
  } else {
    ps->ip = parseOpStatement(activeStack().getIndex(),
                              activeStack().getIndex(), "string", 0, ps);
  }

  activeStack().nextIndex();
  if ((tokens->at(activeStack().getIndex())).value == "(") {
    int cindex = activeStack().getIndex();
    ps->socket = parseOpStatement(
        cindex, reachToken(")", false, true, true, true, true), "int", 0, ps);
  } else {
    ps->socket = parseOpStatement(activeStack().getIndex(),
                                  activeStack().getIndex(), "int", 0, ps);
  }
  return ps;
};
...
Parser::parseStatement(const std::string starting_rule,
  ...
  result = std::shared_ptr<Statement>(
      ebnf.parseStatement(result, starting_rule, custom_callback));
  decreaseScope();
  return result;
}
...
std::shared_ptr<Statement> Parser::parseReset() {
  ...
  if (!peekToken(";")) {
    activeStack().popToken();
    int cindex = activeStack().getIndex();
    rs->count = parseOpStatement(
        cindex, (reachToken(";", true, true, true, false, false) - 1),
        getType(activeStack().getIndex() - 1), 0, rs);
  }
  return rs;
};
...
int Parser::pushDependency(idInfo &id) {
  ...
  // FIX109 a modified varaible in current scope, we dont need to receive it
  // then.
  ...
  for (uint32_t i = 0; i < distStatementTemp->modifiedVariables->size(); i++) {
    ...
    if (distStatementTemp->modifiedVariables->at(i).name == id.name &&
      ...
      if (!id.array) {
        if (id.arrayIndex == NULL && !id.channel && !id.backProp &&
            !id.noblock) {
          return 1;
        }
      }
  ...
  for (uint32_t i = 0; i < distStatementTemp->dependenciesVariables->size();
       i++) {
    string listParentName;

    if (distStatementTemp->dependenciesVariables->at(i).parent) {
      listParentName =
          distStatementTemp->dependenciesVariables->at(i).parent->name;
    }
  ...
  for (uint32_t i = 0; i < currentStatement->distStatements->size(); i++) {
    ...
    if (currentStatement->distStatements->at(i)->variable.parent) {
      listParentName =
          currentStatement->distStatements->at(i)->variable.parent->name;
    }
  ...
  std::shared_ptr<distributingVariablesStatement> dvStatement(
      new distributingVariablesStatement());
  ...
  if (id.channel) {
    ...
    for (int32_t i = distModifiedGlobal->size() - 1; i >= 0; i--) {
      ...
      // cout << "Name : " << distModifiedGlobal->at(i).name << ":" << id.name
      // << endl; cout << "Parents : " << listParentName << ":" << parentName <<
      // endl; cout << "Parents : " <<
      // distModifiedGlobal->at(i).distributedScope << endl;
      if (distModifiedGlobal->at(i).name == id.name &&
          listParentName == parentName) {
        if (id.backProp &&
            distModifiedGlobal->at(i).distributedScope <= id.distributedScope) {
          ...
          break;
        } else if (distModifiedGlobal->at(i).distributedScope !=
                   id.distributedScope) {
          dvStatement->dependencyNode =
              distModifiedGlobal->at(i).distributedScope;
          break;
        }
      }
    }
...
int Parser::pushModified(const string &op, idInfo &id) {
  ...
  if (id.name == currentFunction.returnID ||
      getTopParent(std::shared_ptr<idInfo>(&id))->name ==
          currentFunction.returnID) {
    return 1;
  }
  ...
  if (op == "=" || op == "+=" || op == "-=" || op == "*=" || op == "/=" ||
    ...
    // also make sure it is not modified twice in the same op Statement like : A
    // = B + C(A), where C() will modify A use same code like below(for array
    // index ? )
    ...
    for (uint32_t i = 0; i < distStatementTemp->dependenciesVariables->size();
      ...
      if (id.parent && distStatementTemp->dependenciesVariables->at(i).parent) {
        if (id.parent->arrayIndex ==
            distStatementTemp->dependenciesVariables->at(i)
                .parent->arrayIndex) {
      ...
      if (distStatementTemp->dependenciesVariables->at(i).name == id.name
          //				&&distStatementTemp->dependenciesVariables->at(i).parent
          //== id.parent)
          && parentName == listParentName && sameParentArrayIndex &&
          distStatementTemp->dependenciesVariables->at(i).arrayIndex ==
              id.arrayIndex) {
        distStatementTemp->dependenciesVariables->erase(
            distStatementTemp->dependenciesVariables->begin() + i);
      }
    }
...
std::shared_ptr<Statement>
Parser::parseFunctionCallInternal(bool terminated, const string &returnType,
                                  const string &classID) {
  activeStack().popToken();
  std::shared_ptr<functionCall> funcCall(
      new functionCall); // for every comma , call parseOP
  ...
  } else /** we have parameters ! */
  {
    ...
    while (!peekToken(")")) {
      std::shared_ptr<Statement> parameter =
          parseStatement("full-expression-list");
  ...
  if (funcCall->name == currentFunction.name) {
    ...
    if (error) {
      displayError(fName, activeStack().getToken().lineNumber,
                   activeStack().getToken().columnNumber,
                   "1Parameters error for function call : " + funcCall->name);
    }
    ...
    if (!getFunc(funcCall->name, parameters, returnType, classID).name.size()) {
      if (getFunc(funcCall->name, classID).name.size()) {
        ...
        displayError(fName, activeStack().getToken().lineNumber,
                     activeStack().getToken().columnNumber,
                     "2Parameters error for function call : " + funcCall->name);
      } else {
        displayError(
            fName, activeStack().getToken().lineNumber,
            activeStack().getToken().columnNumber,
            "return type error [" + returnType + "] for function call : [" +
                funcCall->name +
                "] expected : " + getFunc(funcCall->name, classID).returnType);
      }
    }
    funcCall->type =
        getFunc(funcCall->name, parameters, returnType, classID).returnType;
  }
...
std::shared_ptr<Statement> Parser::parseForloop() {
  ...
  if (!peekToken(";")) {
    std::shared_ptr<Statement> stmt =
        parseStatement("loop-expression-condition");
    stmt->line = activeStack().getToken().lineNumber;
    if (stmt->StatementType != oStatement &&
        stmt->StatementType != eStatement) {
      displayError(fName, activeStack().getToken().lineNumber,
                   activeStack().getToken().columnNumber,
                   "Expected conditional Statement");
    }
  ...
  if (!peekToken("]")) {
    std::shared_ptr<Statement> stmt =
        parseStatement("loop-expression-step-list");
    stmt->line = activeStack().getToken().lineNumber;
    if (stmt->StatementType != oStatement &&
        stmt->StatementType != eStatement) {
      displayError(fName, activeStack().getToken().lineNumber,
                   activeStack().getToken().columnNumber,
                   "Expected operational Statement");
    }
...
int Parser::addStatementDistributingVariables(std::shared_ptr<Statement> stmt) {
  ...
  for (uint32_t i = 0; i < currentFunction.body->appendBeforeList.size(); i++) {
    ...
    if (std::static_pointer_cast<termStatement>(
            std::static_pointer_cast<distributingVariablesStatement>(
                currentFunction.body->appendBeforeList.at(i))
                ->variable.arrayIndex) != NULL) {
      cerr << currentFunction.body->appendBeforeList.at(i) << endl << flush;
      stmt->distStatements->push_back(
          std::static_pointer_cast<distributingVariablesStatement>(
              currentFunction.body->appendBeforeList.at(i)));
      // currentFunction.body->append_after(dvStatement);
      currentFunction.body->appendBeforeList.erase(
          currentFunction.body->appendBeforeList.begin() + i);
      i--;
    }
  }
  for (uint32_t i = 0; i < currentFunction.body->appendAfterList.size(); i++) {
    if (std::static_pointer_cast<termStatement>(
            std::static_pointer_cast<distributingVariablesStatement>(
                currentFunction.body->appendAfterList.at(i))
                ->variable.arrayIndex) != NULL) {
      stmt->distStatements->push_back(
          std::static_pointer_cast<distributingVariablesStatement>(
              currentFunction.body->appendAfterList.at(i)));
      // currentFunction.body->append_after(dvStatement);
      currentFunction.body->appendAfterList.erase(
          currentFunction.body->appendAfterList.begin() + i);
      i--;
    }
  }
...
std::shared_ptr<Statement>
Parser::parseFunction() // add functions prototypes to userFunctions Array too
                        // :)
{
  ...
  activeStack().popToken();
  RequireValue("(",
               "Expected \"(\" and not " + activeStack().getToken().value +
  ...
  if (!peekToken("->")) {
    while (activeStack().getToken().value != "->") {
      // TODO: FIX 102
      //  should make a Array of declaration Statements and add the Statements
      //  to it, for the compiler to parse them with their initialized values ?
      // declareStatement* stmt
      // =(declareStatement*)parseDeclarationInternal("->"); declareStatement*
      // stmt =(declareStatement*)parseStatement("Statement");

      std::shared_ptr<declareStatement> stmt =
          std::static_pointer_cast<declareStatement>(parseStatement(
              "declaration-Statement", &Parser::parseDeclarationInternal));
      ...
      for (uint32_t i = 0; i < stmt->identifiers->size(); i++) {
        ...
        Funcinfo.parameters->push_back(
            pair<string, bool>(Id.type, stmt->value));
      }
    ...
    while (activeStack().getToken().value != ")") {
      ...
      for (uint32_t i = 0; i < stmt->identifiers->size(); i++) {
        ...
        if (stmt->identifiers->size() > 1) {
          displayWarning(fName, activeStack().getToken().lineNumber,
                         activeStack().getToken().columnNumber,
                         "Function : " + Funcinfo.name +
                             " : more than one variable for return, only the "
                             "first will be used");
          break;
        }
      }
  ...
  if (activeStack().getToken().value == ";") {
        ...
        if (functionsInfo->at(i).name == Funcinfo.name) {
          if (*functionsInfo->at(i).parameters == *Funcinfo.parameters) {
            if (functionsInfo->at(i).protoType) {
              displayError(fName, activeStack().getToken().lineNumber,
                           activeStack().getToken().columnNumber,
                           "Pre-defined function prototype : " + Funcinfo.name);
            } else // if(function prototype already exists)
            {
              displayError(fName, activeStack().getToken().lineNumber,
                           activeStack().getToken().columnNumber,
                           "Pre-defined function, no need for prototype : " +
                               Funcinfo.name);
            }
          }
        }
      ...
      for (uint32_t i = 0; i < functionsInfo->size(); i++) {
        if (functionsInfo->at(i).name == Funcinfo.name &&
          ...
          if (!functionsInfo->at(i).protoType) {
            displayError(fName, activeStack().getToken().lineNumber,
                         activeStack().getToken().columnNumber,
                         "Pre-defined function : " + Funcinfo.name);
          } else {
            distributedVariablesCount -=
                functionsInfo->at(i).parameters->size() + 1;
    ...
    std::shared_ptr<Array<std::shared_ptr<Statement>>> declarations(
        new Array<std::shared_ptr<Statement>>());
    ...
    while (!peekToken("}")) {
      ...
      if (stmt == NULL) {
        displayError(fName, activeStack().getToken().lineNumber,
                     activeStack().getToken().columnNumber,
                     "error parsing function : " + Funcinfo.name);
      }

      if (stmt->StatementType == dStatement) {
        if (std::static_pointer_cast<declareStatement>(stmt)->Initilazed &&
            !std::static_pointer_cast<declareStatement>(stmt)->global) {
          ...
          // FIX1001 if function is distributed , else put it in
          // pushModified(os->op, id); currentFunction.body
          ...
          for (uint32_t i = 0; i < decStatement->identifiers->size(); i++) {
            ...
            std::shared_ptr<operationalStatement> os(
                new operationalStatement());
            os->left = std::shared_ptr<Statement>(
                new termStatement(decStatement->identifiers->at(i).name));
            os->op = "=";
            os->right = value;
            os->type = stmt->type;
            os->scope =
                std::static_pointer_cast<declareStatement>(decStatement)->scope;

            idInfo id(decStatement->identifiers->at(i).name, os->scope,
                      os->type, NULL);
          ...
          std::shared_ptr<Statement> stmt2(
              currentFunction.body->at(currentFunction.body->size() -
                                       (decStatement->identifiers->size())));
      ...
      if ((dvList->at(i))->type == distributingVariablesStatement::DEPS &&
        ...
        for (uint32_t k = i + 1; k < dvList->size(); k++) {
          if ((dvList->at(k))->type != distributingVariablesStatement::MODS ||
              ((dvList->at(k))->variable.distributedScope ==
               (dvList->at(i))->variable.distributedScope) ||
              ((dvList->at(k))->variable.name !=
               (dvList->at(i))->variable.name)) {
            continue;
          }
          dvList->at(i)->dependencyNode =
              (dvList->at(k))->variable.distributedScope;
          break;
        }
      }
    ...
    // TODO: here :D fix all functions to fill the type field , and add type to
    // functionInfo too
    functionsInfo->push_back(Funcinfo);

    // TODO:before this, loop through origianl body and push the modfifyNotify
    // Statements at their right position i

    // TODO:fix what if , two initiated variables are pushed at the same place ?

    std::shared_ptr<termStatement> statement(
        new termStatement("DM14FUNCTIONBEGIN", "NIL"));
    statement->scope = scope;
...
int Parser::findIDInfo(const idInfo &ID, const int &returnType) {
  for (uint32_t i = 0; i < identifiers->size(); i++) {
    // if(ID.name ==(identifiers->at(i)).name && ID.parent && ID.parent->name
    // ==(identifiers->at(i)).parent->name)
    ...
    if (ID.name == (identifiers->at(i)).name &&
        parentName == identifiersParentName) {
      if (returnType == SCOPE) {
...
bool Parser::isFunction(const string &func, bool recursiveCurrent,
                        const string &classID) {
  if (isUserFunction(func, recursiveCurrent, classID) ||
...
bool Parser::isUserFunction(const string &func, bool recursiveCurrent,
                            const string &classID) {
  if (!classID.size()) {
    if (recursiveCurrent) {
    ...
    for (uint32_t i = 0; i < functionsInfo->size(); i++) {
      std::cerr << "function name : " << (functionsInfo->at(i)).name
                << std::endl;
      if (func == (functionsInfo->at(i)).name) {
...
std::shared_ptr<Statement>
Parser::findTreeNode(std::shared_ptr<Statement> opStatement,
                     int StatementType) {
  if (!opStatement) {
    return nullptr;
  }
  ...
  if (opStatement->StatementType == oStatement) {
    ...
    if (std::static_pointer_cast<operationalStatement>(opStatement)->left) {
      result = findTreeNode(
          std::static_pointer_cast<operationalStatement>(opStatement)->left,
          StatementType);
    }

    if (!result &&
        std::static_pointer_cast<operationalStatement>(opStatement)->right) {
      result = findTreeNode(
          std::static_pointer_cast<operationalStatement>(opStatement)->right,
          StatementType);
    }
...
int32_t Parser::searchVariables(std::shared_ptr<Statement> opStatement,
                                int depencyType, string op) {
  ...
  if (opStatement->StatementType == tStatement) {
    ...
    std::shared_ptr<idInfo> id =
        std::static_pointer_cast<termStatement>(opStatement)->id;
    if (id) {
  ...
  if (opStatement->StatementType == oStatement) {
    if (std::static_pointer_cast<operationalStatement>(opStatement)->left) {
      if (std::static_pointer_cast<operationalStatement>(opStatement)->op ==
              "." ||
          std::static_pointer_cast<operationalStatement>(opStatement)->op ==
              "::") {
        if (std::static_pointer_cast<operationalStatement>(opStatement)
                ->left->StatementType == oStatement) {
          std::shared_ptr<Statement> leftLeft =
              std::static_pointer_cast<operationalStatement>(
                  std::static_pointer_cast<operationalStatement>(opStatement)
                      ->left)
                  ->left;
...
std::shared_ptr<Statement> Parser::parseExpressionStatement() {
  ...
  if (activeStack().Tokens()->at(activeStack().Tokens()->size() - 1).value ==
      ";") {
    result = parseOpStatement(0, activeStack().Tokens()->size() - 2, "-2",
                              scope, parentStatement);
  } else {
    result = parseOpStatement(0, activeStack().Tokens()->size() - 1, "-2",
                              scope, parentStatement);
  }
  return result;
}
...
std::string Parser::getOpStatementType(std::string stmtType,
  ...
  if (stmtType == "NIL") {
    displayError(fName, activeStack().getToken().lineNumber,
                 activeStack().getToken().columnNumber,
                 "Wrong type variable : " + activeStack().getToken().value);
  } else if (stmtType == "-2") {
    if (isFunction(activeStack().getToken(0).value, true,
                   classID)) /// function, set to it's return type
    {
      type = getFunc(activeStack().getToken(0).value, classID).returnType;
    } else if (DM14::types::isDataType(activeStack()
                                           .getToken(0)
                                           .value)) /// if it is  datatype, use
                                                    /// it as the Statement type
    {
      type = activeStack().getToken(0).value;
    } else if (activeStack().getToken(0).type ==
               "identifier") // if identifier, then use it's type
    {
      type = findIDType(idInfo(activeStack().getToken(0).value, 0, "", NULL),
                        classID);
    } else if (DM14::types::isImmediate(
                   activeStack().getToken(0))) /// use immediate type
    {
      type = activeStack().getToken(0).type;
    }
    /*else if((DM14::types::isSingleOperator(activeStack().getToken(0).value) ||
    activeStack().getToken(0).value == "@") && 0 < to)
    {
            ...
            {
                    type  = findIDType(idInfo(activeStack().getToken(0+1).value,
    0, "", NULL), classID);
            }
            else
            {
                    displayError(fName,
    activeStack().getToken(0).lineNumber,activeStack().getToken(0).columnNumber,"Did
    not expect "+ activeStack().getToken(0).value);
            }
    }
    else
    {
            displayError(fName, activeStack().getToken(0).lineNumber,
    activeStack().getToken(0).columnNumber,"Did not expect "+
    activeStack().getToken(0).value);
    }*/
  } else {
    type = stmtType;
  }
...
std::shared_ptr<std::vector<token>> Parser::extract(int32_t from, int32_t to) {
  std::shared_ptr<std::vector<token>> extract_temp_vector(
      new std::vector<token>());
  for (uint32_t i = 0; i < from; i++) {
    extract_temp_vector->push_back(activeStack().popToken());
  }

  for (uint32_t i = to + 1, current_size = activeStack().Tokens()->size();
       i < current_size; i++) {
...
std::shared_ptr<Statement> Parser::parseOpStatement(
    int32_t from, int32_t to, const string &stmtType, const int &scopeLevel,
    std::shared_ptr<Statement> caller, std::shared_ptr<idInfo> parent,
    const string &parentOp) {
  ...
  if (plevel > 0) {
    displayError(fName, activeStack().getToken().lineNumber,
                 activeStack().getToken().columnNumber, " missing \")\" ");
  } else if (plevel < 0) {
    displayError(fName, activeStack().getToken().lineNumber,
                 activeStack().getToken().columnNumber, " missing \"(\" ");
  }
  ...
  // big(..) Statement
  if (activeStack().getToken(from).value == "(" &&
      activeStack().getToken(to).value == ")" && removeBigParenthes) {
    activeStack().popToken();
    ...
    if (plevel == 0) {
      if ((activeStack().getToken(i).type == "operator") &&
        ...
        if (i != from) /** not a prefix operator like ++x */
        {
          ...
          opStatement->left =
              parseOpStatement(from, i - 1, stmtType, opStatement->scopeLevel,
                               currentStatement, parent, opStatement->op);
        ...
        std::shared_ptr<Statement> res =
            findTreeNode(opStatement->left, tStatement);
        ...
        if (opStatement->op == "." || opStatement->op == "::") {
          if (!id) {
            displayError(fName, activeStack().getToken(i).lineNumber,
                         activeStack().getToken(i).columnNumber,
                         "false class member !");
            ...
            if (i == to && to > 0) {
              displayError(fName, activeStack().getToken(i).lineNumber,
                           activeStack().getToken(i).columnNumber,
                           "incomplete data member access at " +
                               activeStack().getToken(i).value);
            }

            opStatement->right = parseOpStatement(
                from, to, "-2", opStatement->scopeLevel, currentStatement, id);
            ...
            if (DM14::types::isDataType(classID) && opStatement->op != "::") {
              std::shared_ptr<Statement> staticFunction =
                  findTreeNode(opStatement->right, fCall);
              if (staticFunction != nullptr) {

                auto fInfoResult = DM14::types::getClassMemberFunction(
                    classID,
                    *std::static_pointer_cast<functionCall>(staticFunction));
                // TODO
                if (fInfoResult.first == true) {
                  ...
                  displayError(
                      fName, activeStack().getToken(i).lineNumber,
                      activeStack().getToken(i).columnNumber,
                      "class " + classID + " has no member static function " +
                          std::static_pointer_cast<functionCall>(staticFunction)
                              ->name);
                }
              } else {
                std::shared_ptr<Statement> staticVariable =
                    findTreeNode(opStatement->right, tStatement);
                if (staticVariable != nullptr) {
                  auto vInfoResult = DM14::types::getClassMemberFunction(
                      classID,
                      *std::static_pointer_cast<functionCall>(staticVariable));
                  if (vInfoResult.first == true) {
                    ...
                    displayError(fName, activeStack().getToken(i).lineNumber,
                                 activeStack().getToken(i).columnNumber,
                                 "class " + classID +
                                     " has no member static variable " +
                                     std::static_pointer_cast<functionCall>(
                                         staticVariable)
                                         ->name);
                  }
                } else {
                  displayError(fName, activeStack().getToken(i).lineNumber,
                               activeStack().getToken(i).columnNumber,
                               "parsing error, should not be here !");
                }
              }
            }
            ...
            /*if(stmtType != "-2" && !hasTypeValue(stmtType,
            findIDType(idInfo(variableName, 0, "", NULL)) ))
            {
                    displayError(fName,(tokens->at(i+2)).lineNumber,(tokens->at(i+2)).columnNumber,"Wrong
            type variable : " + variableName + " expected : " +
            opStatement->type);
            }*/
          }
        } else if (to > -1) {
          opStatement->right = parseOpStatement(
              from, to, opStatement->type == "" ? stmtType : opStatement->type,
              opStatement->scopeLevel, currentStatement);
          opStatement->absorbDistStatements(opStatement->right);
          if (!opStatement->left) {
            opStatement->type = opStatement->right->type;
          }
        ...
        if (DM14::types::isbinOperator(opStatement->op) && to > -1) {
          ...
          // FIXME:
          /*else if(caller && caller->StatementType == oStatement && !parent &&
          !typeHasOperator(((operationalStatement*)caller)->op,
          opStatement->type) && opStatement->op != "::"  )
          {
                  cerr  << opStatement->op << endl;
                  cerr << tokens->at(from).value << endl;
                  cerr << tokens->at(to).value << endl;
                  displayError(fName,(tokens->at(from)).lineNumber,(tokens->at(from+2)).columnNumber,"typee
          \"" +opStatement->type + "\" does not support operator "
          +((operationalStatement*)caller)->op);
          }*/
          else if (!DM14::types::typeHasOperator(opStatement->op,
                                                 opStatement->type) &&
                   opStatement->op != "." && opStatement->op != "::") {
            if (opStatement->left)
            ...
            displayError(fName, activeStack().getToken().lineNumber,
                         activeStack().getToken().columnNumber,
                         "type \"" + opStatement->type +
                             "\" does not support operator " + opStatement->op);
          }

          searchVariables(opStatement->left,
                          distributingVariablesStatement::MODS,
                          opStatement->op);
          searchVariables(opStatement->right,
                          distributingVariablesStatement::DEPS);
        } else if (DM14::types::isSingleOperator(opStatement->op)) {
          if (!DM14::types::typeHasOperator(opStatement->op,
                                            opStatement->type) &&
              (opStatement->op != "@")) {
            displayError(fName, activeStack().getToken().lineNumber,
                         activeStack().getToken().columnNumber,
          ...
          if (opStatement->op == "@") {
            ...
            if (res) {
              ...
              if (!isDistributed) {
                displayError(fName, activeStack().getToken().lineNumber,
                             activeStack().getToken().columnNumber,
                             activeStack().getToken().value +
                                 " : can not use @ operator with a "
                                 "non-distributed variable");
                // displayError("can not use @ operator with a non-distributed
                // variable");
              }
            } else {
            ...
            // variable@node
            if (opStatement->left &&
                opStatement->left->StatementType == tStatement) // variable@node
            {
              ...
              if (opStatement->right->StatementType == oStatement) {
                std::shared_ptr<operationalStatement> stmt =
                    std::static_pointer_cast<operationalStatement>(
                        opStatement->right);
                searchVariables(opStatement->left,
                                distributingVariablesStatement::MODS, stmt->op);
                searchVariables(opStatement->left,
                                distributingVariablesStatement::DEPS);
            ...
            {
              // remove ny depedency of this the variable as we only need its
              // address and we don't access the variable ittself !
              for (uint32_t l = 0; l < currentStatement->distStatements->size();
                   l++) {
                if (currentStatement->distStatements->at(l)->type ==
                    distributingVariablesStatement::DEPS) {
                  if (currentStatement->distStatements->at(l)->variable.name ==
                      rightId->name) {
          ...
          displayError("Operator is not defined as single or binary  : " +
                       opStatement->op);
        }
        ...
        if (!DM14::types::typeHasOperator(opStatement->op, opStatement->type) &&
            ...
            opStatement->op != "@") {
          // displayError(fName,(tokens->at(from)).lineNumber,(tokens->at(from+2)).columnNumber,"type
          // \"" +opStatement->type + "\" does not support operator " +
          // opStatement->op);
        }

        /*
        if(!hasTypeValue(opStatement->type, opStatement->right->type) &&
        !classMember && stmtType!= "-2")
        {
                displayError(fName,(tokens->at(from)).lineNumber,(tokens->at(from+2)).columnNumber,"Wrong
        operands type : " + opStatement->right->type + " with "
        +opStatement->type);
        }
        else if(!hasTypeValue(stmtType, opStatement->type) && stmtType != "-2")
        {
                displayError(fName,(tokens->at(from)).lineNumber,(tokens->at(from+2)).columnNumber,"Wrong
        operands type : " + stmtType + " with " +opStatement->type);
        }*/
        return opStatement;
      }
    }
  ...
  if (activeStack().getToken(0).value.size() == 0) // consumed all tokens !
  {
    ...
    displayError(fName, activeStack().getToken(0).lineNumber,
                 activeStack().getToken(0).columnNumber,
  ...
  if (stmtType != "-2" && stmtType != type) {
    displayError(fName, activeStack().getToken(0).lineNumber,
                 activeStack().getToken(0).columnNumber,
                 "types mismatch ! " + activeStack().getToken(0).value);
  }

  opStatement->type = type;

  if (to == -1 ||
      activeStack().getToken(0).value.size() == 0) // consumed all tokens !
  {
    return opStatement;
  }
  ...
  // if(!typeHasOperator(tokens->at(currentIndex).value,
  // findID(tokens->at(currentIndex+1).value, classID).type))
  //{
  //	displayError(fName,(tokens->at(currentIndex)).lineNumber,(tokens->at(currentIndex)).columnNumber,"single
  //operator " +(tokens->at(currentIndex)).value + " is not supported by
  //Statements of type " + findID(tokens->at(currentIndex+1).value,
  //classID).type);
  //	}
  ...
  if (activeStack().getToken(from).type == "identifier" ||
      DM14::types::isDataType(activeStack().getToken().value)) //(tokens->at(currentIndex)).type
                                                               //== "datatype")
  {
    // function !
    if (isBuiltinFunction(activeStack().getToken(from).value) ||
        isUserFunction(activeStack().getToken(from).value, true) ||
        (parent && DM14::types::classHasMemberFunction(
                       classID, activeStack().getToken(from).value))) {
      ...
      if (parent) {
        if (opStatement->type != classID) {
          ...
          stmt->type = DM14::types::classMemberFunctionType(
              classID, activeStack().getToken(from).value);
        }
      } else {
    ...
    // variable !
      ...
      if (DM14::types::isDataType(variableName)) {
        displayInfo(fName, activeStack().getToken().lineNumber,
                    activeStack().getToken().columnNumber, "FIX");
      } else if (parent &&
                 !DM14::types::classHasMemberVariable(classID, variableName)) {
        displayError(fName, activeStack().getToken().lineNumber,
                     activeStack().getToken().columnNumber,
                     "class " + classID + " has no member : " + variableName);
      } else if (parent &&
                 DM14::types::classHasMemberVariable(classID, variableName)) {
        if (DM14::types::getClassMemberVariable(classID, variableName)
                .second.classifier != DM14::types::CLASSIFIER::PUBLIC) {
          displayError(
              fName, activeStack().getToken().lineNumber,
              activeStack().getToken().columnNumber,
              variableName +
                  " is a private class member variable of class type : " +
                  classID);
        }
      } else if (findIDType(idInfo(variableName, 0, "", NULL)) == "NIL" &&
                 !parent) {
        if (peekToken(0).value == "(") {
          displayError(fName, activeStack().getToken().lineNumber,
                       activeStack().getToken().columnNumber,
                       "Un-defined function : " + variableName);
        } else {
          displayError(fName, activeStack().getToken().lineNumber,
                       activeStack().getToken().columnNumber,
                       "Un-defined variable : " + variableName);
        }
      }
      ...
      std::shared_ptr<termStatement> statement(
          new termStatement(variableName, opStatement->type));
      ...
      statement->size =
          findIDInfo(idInfo(variableName, 0, "", NULL), ARRAYSIZE);
      ...
      if (activeStack().getToken(0).value == "[") {
        if (!findIDInfo(idInfo(variableName, 0, "", NULL), ARRAY)) {
          displayError(fName, activeStack().getToken().lineNumber,
                       activeStack().getToken().columnNumber,
                       variableName +
                           " is not an array, invalid use of indexing");
        }
        // termStatement->arrayIndex =
        // parseConditionalExpression(currentStatement);
      ...
      // idInfo* id = new idInfo(termStatement->term, opStatement->scope,
      // opStatement->type, aIndex);
      std::shared_ptr<idInfo> id(new idInfo(statement->term, opStatement->scope,
                                            statement->type, aIndex));
      id->distributedScope = distributedScope;
      if (parent) {
        id->parent = parent;
      }
      ...
      if (parentOp != "=" && parentOp != "+=" && parentOp != "-=" &&
          ...
          parentOp != "--" && parentOp != "@") {
        if (peekToken(0).value != "." &&
            peekToken(0).value != "::") // do not pus class id, since it will be
                                        // done by the member anyway !
        {
          pushDependency(*id);
        }
      }
    ...
    if (!DM14::types::hasTypeValue(opStatement->type,
                                   activeStack().getToken(from).type)) {
      displayError(fName, activeStack().getToken(from).lineNumber,
                   activeStack().getToken(from).columnNumber,
                   "wrong immediate type : " +
                       activeStack().getToken(from).value);
    }

    activeStack().popToken();

    std::shared_ptr<termStatement> statement(
        new termStatement(activeStack().getToken().value, opStatement->type));
    statement->line = activeStack().getToken().lineNumber;
    statement->scope = scope;
    statement->scopeLevel = scopeLevel;
...
funcInfo Parser::getFunc(const string &funcID, Array<string> *mparameters,
                         const string &returnType, const string &classID) {
  funcInfo finfo;
  ...
  if (classID.size()) {
    ...
    for (uint32_t i = 0; i < datatypes.size(); i++) {
      if (datatypes.at(i).typeID == classID && datatypes.at(i).classType) {
        for (uint32_t k = 0; k < datatypes.at(i).memberFunctions.size(); k++) {
          if (datatypes.at(i).memberFunctions.at(k).returnType != returnType &&
              returnType.size()) {
            if (!DM14::types::hasTypeValue(
                    returnType,
                    datatypes.at(i).memberFunctions.at(k).returnType)) {
              continue;
            }
          }

          if (datatypes.at(i).memberFunctions.at(k).name == funcID) {
            if (!datatypes.at(i).memberFunctions.at(k).parameters->size() &&
                !mparameters->size()) {
              return datatypes.at(i).memberFunctions.at(k);
            }

            for (uint32_t l = 0; l < mparameters->size(); l++) {
              if (mparameters->at(l) != datatypes.at(i)
                                            .memberFunctions.at(k)
                                            .parameters->at(l)
                                            .first) {
                // cerr << mparameters->at(l) << ":" <<
                // datatypes.at(i).memberFunctions.at(k).parameters->at(l).first
                // << endl; cerr << datatypes.at(i).memberFunctions.at(k).name
                // << ":" <<  funcID << endl;
                break;
              }
              ...
              if (l == mparameters->size() - 1) {
                if (l ==
                    datatypes.at(i).memberFunctions.at(k).parameters->size() -
                        1) {
                  finfo = datatypes.at(i).memberFunctions.at(k);
                  break;
                } else if (datatypes.at(i)
                               .memberFunctions.at(k)
                               .parameters->at(l + 1)
                               .second) {
                  finfo = datatypes.at(i).memberFunctions.at(k);
    ...
    for (uint32_t i = 0; i < functionsInfo->size(); i++) {
      if (functionsInfo->at(i).returnType != returnType && returnType.size()) {
        if (!DM14::types::hasTypeValue(returnType,
                                       functionsInfo->at(i).returnType)) {
          continue;
        }
      }
      ...
      if ((functionsInfo->at(i)).name == funcID) {
        ...
        // cerr << functionsInfo->at(i).parameters->size() << ":" <<
        // mparameters->size() << endl;
        for (uint32_t k = 0, l = 0;
             k < functionsInfo->at(i).parameters->size() &&
             l < mparameters->size();
             k++, l++) {
          // cerr << mparameters->at(l)  << ":" <<
          // functionsInfo->at(i).parameters->at(k).first << endl << flush;
          if (mparameters->at(l) !=
              functionsInfo->at(i).parameters->at(k).first) {
            break;
          }
...
mapcode::mapcode(const string &mname,
                 std::shared_ptr<Array<ast_function>> const mfunctions,
...
Array<includePath> Parser::getIncludes() { return includes; };

std::shared_ptr<Array<ast_function>> Parser::getFunctions() {
  return functions;
};

std::shared_ptr<Array<mapcode>> Parser::getMapCodes() { return mapCodes; };

std::shared_ptr<Array<string>> Parser::getExternCodes() { return ExternCodes; }

string mapcode::getFileName() { return fileName; };
...
Array<includePath> mapcode::getIncludes() { return includes; };

bool mapcode::isHeader() { return Header; };
...
std::shared_ptr<Statement> Parser::parseThread() {
  ...
  stringstream SS;
  SS << activeStack().getToken().columnNumber
     << activeStack()
            .getToken()
            .lineNumber; // Number of character on the current line
  thread->Identifier = SS.str();

  if (parentStatement && (parentStatement->StatementType == fLoop ||
                          parentStatement->StatementType == wLoop)) {
    displayWarning(fName, activeStack().getToken().lineNumber,
                   activeStack().getToken().columnNumber,
                   "Thread call inside a loop ! careful ");
  }
  ...
  for(int32_t i= 0; i < working_tokens->size(); i++)
  {
          ...
          if(tokens->at(i).type == "identifier" && findID(tokens->at(i).value,
  parent).name.size() && !findIDInfo(idInfo((tokens->at(i)).value, 0, "", NULL),
  GLOBAL))
          {
                  displayError(fName, activeStack().getToken(0).lineNumber,
  activeStack().getToken(0).columnNumber,"Only global variables are allowed to
  be passed to a thread call ! , " + activeStack().getToken(0).value + " is not
  global");
          }
  }
...
std::shared_ptr<Statement> Parser::parseStruct() {
  ...
  while (!peekToken("}")) {
    std::shared_ptr<declareStatement> stmt =
        std::static_pointer_cast<declareStatement>(parseStatement("Statement"));
...
int Parser::extractSplitStatements(
...
std::shared_ptr<Statement> Parser::parseCase() {
  ...
  while (!peekToken("}")) {
    ...
    while (tokens->at(activeStack().getIndex()).value != "}" &&
           tokens->at(activeStack().getIndex()).value != "[") {
      Case->Body[CCondition].push_back(parseStatement("Statement"));
      if (!peekToken("}") && !peekToken("[")) {
        activeStack().nextIndex();
      } else {
        // nextIndex(); // if we add this, then we might stop at [, which needs
        // to back a
        break;
      }
    }
...
int32_t
Parser::reachToken(const string &Char, const bool &sameLine,
                   const bool &reportError, const bool &actual,
                   const bool &beforeEOF,
                   const bool &scopeLevel) // loop till find the specified char
{
  ...
  if (actual) {
    while (activeStack().popToken().value.size()) {
      ...
      if ((beforeEOF && activeStack().getToken().value == ";") ||
          activeStack().getToken().value == Char) {
        if (scopeLevel) {
          if (plevel == 0) {
    ...
    while ((unsigned)iterator < activeStack().Tokens()->size()) {
      if (sameLine) {
        if (activeStack().getToken(iterator).lineNumber != line) {
          if (reportError) {
            displayError(fName, activeStack().getToken(iterator).lineNumber,
                         activeStack().getToken(iterator).columnNumber,
                         "Missing " + Char);
          } else {
            return -1;
          }
      ...
      if ((beforeEOF && activeStack().getToken().value == ";") ||
          activeStack().getToken(iterator).value == Char) {
        return iterator;
      }
...
/**
 * @detail peek the first token in the working_tokens vector, which is not poped
 * yet
 */
...
string Parser::getType(const int &Index) {
  if (isIdentifier(activeStack().getToken().value)) {
    ...
    displayError(fName, activeStack().getToken().lineNumber,
                 activeStack().getToken().columnNumber,
                 "can not get type of : " + activeStack().getToken().value);
  }
  return "NIL";
};
...
bool Parser::checkToken(int type, string error, bool addtoken) {
  ...
  if (type == DM14::types::types::Function) {
    if (isBuiltinFunction(activeStack().getToken().value) ||
        isUserFunction(activeStack().getToken().value, true)) {
      rError = true;
    }
  ...
  if (rError) {
    displayError(fName, activeStack().getToken().lineNumber,
                 activeStack().getToken().columnNumber,
                 error + activeStack().getToken().value);
  }
  return rError;
};
...
bool Parser::RequireType(
    string type, string error,
    bool addtoken) // add and int for index modification like 1 or -1 :D
{
  bool rError = false;
  ...
  if (rError) {
    if (addtoken) {
      ...
      displayError(fName, activeStack().getToken().lineNumber,
                   activeStack().getToken().columnNumber, error);
    }
  }
  return rError;
};

bool Parser::RequireValue(
...
// maybe w should move this to scanner ? and modify isFunction instead of
// reading the stupid manually made maps file !?!
int Parser::mapFunctions(const string &package, const string &library) {
  ...
  for (uint32_t i = 0; i < includePaths.size(); i++) {
    ...
    if (library == package) {
      fullLibraryName =
          includePaths.at(i) + "/" + package + "/" + package + ".hpp";
    } else {
      fullLibraryName =
          includePaths.at(i) + "/" + package + "/" + library + ".hpp";
    }
  ...
  if (!scner->isReady()) {
    displayError(scner->getFileName(), -1, -1,
                 "Could not open Library : " + fullLibraryName);
  }
  ...
  for (uint32_t i = 0; i < mapTokens->size(); i++) {
    ...
    } else if (mapTokens->at(i).value == "using") // using
    {
      if (scner->reachToken(i, ";", false, false, true) == -1) {
        displayError(
            scner->getFileName(), -1, -1,
            "Internal Compiler error, parsing using Statement in package : " +
                fullLibraryName);
      }
      i = scner->reachToken(i, ";", false, false, true);
    } else if (mapTokens->at(i).value == "template" ||
               mapTokens->at(i).value == "typename") {
      int brace = 0;
      for (uint32_t k = i + 1; k < mapTokens->size(); k++) {
        if (mapTokens->at(k).value == "class" ||
            mapTokens->at(k).value == "typename") {
               ...
               mapTokens->at(i).value == "typedef") // class , struct, typedef
    {
      ...
      if (i == 0) {
        displayError(scner->getFileName(), -1, -1,
                     "Internal Compiler error, parsing class in package : " +
                         fullLibraryName);
      }
    } else if (DM14::types::isDataType(mapTokens->at(i).value) ||
               mapTokens->at(i).value == "void" ||
      ...
      displayError(
          scner->getFileName(), -1, -1,
          "Internal Compiler error, parsing package : " + fullLibraryName +
              " : token -> " + mapTokens->at(i).value);
    }
  }
...
long Parser::parseCClass(std::shared_ptr<DM14::scanner> scner, uint32_t start,
                         const Array<string> &templateNames) {
  DatatypeBase CClass;
  ...
  if (!forwardDecelation) {
    for (uint32_t i = start; i < mapTokens->size(); i++) {
      if (mapTokens->at(i).value == "public" ||
          mapTokens->at(i).value == "public:") {
        if (mapTokens->at(i).value == "public") {
          if (scner->reachToken(i, ":", false, false, true) != i + 1) {
            displayError(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                         (mapTokens->at(i)).columnNumber,
                         "Expected : inside class declaration : " +
                             CClass.typeID + package);
          }
        }
        classifier = DM14::types::CLASSIFIER::PUBLIC;
        i++;
      } else if (mapTokens->at(i).value == "private" ||
                 mapTokens->at(i).value == "private:") {
        ...
        i++;
      } else if (mapTokens->at(i).value == "protected" ||
                 mapTokens->at(i).value == "protected:") {
  ...
  if (!protoType) {
    for (uint32_t i = 0; i < datatypes.size(); i++) {
      if (datatypes.at(i).typeID == CClass.typeID &&
          datatypes.at(i).classType && datatypes.at(i).protoType) {
        datatypes.remove(i);
        i--;
      }
    }
...
funcInfo Parser::parseCFunction(std::shared_ptr<DM14::scanner> scner,
                                uint32_t start,
                                const DatatypeBase &parentClass) {
  funcInfo funcinfo;
  std::shared_ptr<Array<token>> mapTokens = scner->getTokens();
  ...
  bool Enum = false;
  for (uint32_t i = start; i < mapTokens->size();
       i++) // function return, data member type
  {

    if (mapTokens->at(i).value == "const" ||
        mapTokens->at(i).value == "unsigned" || mapTokens->at(i).value == "&" ||
        mapTokens->at(i).value == "struct" ||
        mapTokens->at(i).value == "volatile") {
      continue;
      ...
      break;
               ...
               mapTokens->at(i).value == "typename") {
      for (uint32_t k = i + 1; k < mapTokens->size();
           k++) // skip tempalte stuff
      {
      ...
      displayWarning(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                     (mapTokens->at(i)).columnNumber,
                     "Functions should not return a pointer  : " +
                         funcinfo.name + package);
      continue;
    } else {
      ...
      if (mapTokens->at(i).value == "~") {
        destructor = true;
        continue;
      } else if (mapTokens->at(i).type != "identifier" &&
                 !DM14::types::isDataType(
                     mapTokens->at(i)
                         .value)) // mapTokens->at(i).type != "datatype")
      {
        displayError(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                     (mapTokens->at(i)).columnNumber,
                     "could not parse a c++ function retun type, expected "
                     "identifier and not :  " +
                         mapTokens->at(i).value);
      }

      if (parentClass.typeID.size() &&
          mapTokens->at(i).value == parentClass.typeID) {
        if (!destructor) {
        ...
        if (mapTokens->at(i).value == "void") {
          displayWarning(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                         (mapTokens->at(i)).columnNumber,
                         "Functions should not have return type of \"void\" "
                         "adding as int ");
          funcinfo.returnType = "int";
        } else {
          funcinfo.returnType =
              DM14::types::getDataType(mapTokens->at(i).value);

          if (funcinfo.returnType == "NIL") {
            displayError(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                         (mapTokens->at(i)).columnNumber,
                         "Uknown type : " + mapTokens->at(i).value);
          }
        }
      }
  ...
  for (uint32_t i = start + 1; i < mapTokens->size();
       i++) // ignore template parameters
  {
       ...
       i++) // the function/member name
  {
    ...
    if (mapTokens->at(i).value == "(") {
      ...
      break;
               ...
               mapTokens->at(i).type != "keyword") {
      displayError(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                   (mapTokens->at(i)).columnNumber,
                   "could not parse a c++ function/member name, expected "
                   "identifier and not: " +
                       mapTokens->at(i).value);
    }
  ...
  for (uint32_t i = start + 1; i < mapTokens->size();
       i++) // the member variable
  {
    if (mapTokens->at(i).value == ";") {
      if (!funcinfo.name.size()) {
        displayError(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                     (tokens->at(i)).columnNumber,
                     "parsing error : " + mapTokens->at(i).value);
      }
      funcinfo.classifier = i;
  ...
  for (uint32_t i = start + 1; i < mapTokens->size();
       i++) // function parameterss
  {
    if (mapTokens->at(i).value ==
        "::") // function member dfinition,, ignore all
    {
      int depthLevel = 0;
      for (uint32_t k = i + 1; k < mapTokens->size(); k++) {
        if (mapTokens->at(k).value == "{") {
    ...
    if (mapTokens->at(i).value == ")") {
      ...
      if (mapTokens->at(i).value != ";") {
        // displayError(scner->getFileName(),(mapTokens->at(i)).lineNumber,(tokens->at(i)).columnNumber,"parsing
        // error, expected ; :" + mapTokens->at(i).value);
        displayWarning(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                       (mapTokens->at(i)).columnNumber,
                       "Function definition in header, be careful !");
      }
    ...
    for (uint32_t k = i; k < mapTokens->size(); k++) // parameter type
    {
      if (mapTokens->at(k).value == "const" ||
        ...
        displayWarning(scner->getFileName(), (mapTokens->at(k)).lineNumber,
                       (mapTokens->at(k)).columnNumber,
                       "Functions should not take a pointer  : " +
                           funcinfo.name + package);
        continue;
      } else if (mapTokens->at(k).value == ",") {
        i = k;
        ...
        if (mapTokens->at(k).value == "void") {
          displayWarning(
              scner->getFileName(), (mapTokens->at(i)).lineNumber,
              (mapTokens->at(k)).columnNumber,
              "Functions should not have return type of \"void\"  : " +
                  funcinfo.name + package);
        } else if (mapTokens->at(k).type != "identifier") {
          // cout << k << ":" << i << ":" << start<<endl;
          // displayError(scner->getFileName(),(mapTokens->at(k)).lineNumber,(mapTokens->at(k)).columnNumber,"could
          // not parse a c++ function retun type, expected identifier: " +
          // mapTokens->at(i).value);
        }

        if (parentClass.typeID.size() &&
            mapTokens->at(k).value == parentClass.typeID) {
          funcinfo.parameters->push_back(
              pair<string, bool>(parentClass.typeID, false));
        } else {
          if (DM14::types::getDataType(mapTokens->at(k).value) != "NIL") {
            funcinfo.parameters->push_back(pair<string, bool>(
                DM14::types::getDataType(mapTokens->at(k).value), false));
            found = true;
            ...
            displayError(scner->getFileName(), (mapTokens->at(k)).lineNumber,
                         (mapTokens->at(k)).columnNumber,
                         "Uknown type : " + mapTokens->at(k).value);
          }
        }
      }
    }
  ...
  for (uint32_t i = start + 1; i < mapTokens->size();
       i++) // finish variables inizilization :X(x),Y(y),Z(z)
  {
    if (mapTokens->at(i).value == ":") {
      for (uint32_t k = i + 1; k < mapTokens->size(); k++) {
        if (mapTokens->at(k).value == ")") {
       ...
       i++) // function inizliation;
  {
    ...
    if (mapTokens->at(i).value == "{") {
      for (uint32_t k = i + 1; k < mapTokens->size(); k++) {
        ...
        if (depthLevel == 0) {
          ...
          if (scner->tokenAt(k + 1).value != ";") {
            displayError(scner->getFileName(), (mapTokens->at(k)).lineNumber,
                         (mapTokens->at(k)).columnNumber,
                         "Unterminated function definition, missing ;");
          }
      ...
      displayError(scner->getFileName(), (mapTokens->at(i)).lineNumber,
                   (mapTokens->at(i)).columnNumber,
                   "not expecting : " + (mapTokens->at(i)).value);
    }
    break;
  }
...
idInfo Parser::findID(const string &ID, const string &parentID) {
  for (uint32_t i = 0; i < identifiers->size(); i++) {
    // if(ID ==(identifiers->at(i)).name && parentID
    // ==(identifiers->at(i)).parent)
    if ((identifiers->at(i)).parent && parentID.size()) {
      if (ID == (identifiers->at(i)).name &&
          parentID == (identifiers->at(i)).parent->name) {
...
std::shared_ptr<Statement> Parser::parseDeclarationInternal() {
  ...
  while (true) {
    checkToken(DM14::types::types::Function,
               "this is a function name ! :  ", true);
    RequireType("identifier", "Expected \"Identifier\" and not ", true);

    if (findIDInfo(idInfo(activeStack().getToken().value, 0, "", NULL),
                   SCOPE) == decStatement->scope) {
      if (findIDInfo(idInfo(activeStack().getToken().value, 0, "", NULL),
                     TMPSCOPE)) {
        displayWarning(fName, activeStack().getToken().lineNumber,
                       activeStack().getToken().columnNumber,
                       "Pre-defined variable in different scope :  " +
                           activeStack().getToken().value);
      } else if (!tmpScope) {
        displayError(fName, activeStack().getToken().lineNumber,
                     activeStack().getToken().columnNumber,
                     "Pre-defineddd variable :  " +
                         activeStack().getToken().value);
      }
    } else if (findIDInfo(idInfo(activeStack().getToken().value, 0, "", NULL),
                          GLOBAL)) {
      displayError(fName, activeStack().getToken().lineNumber,
                   activeStack().getToken().columnNumber,
    ...
    tempIdentifiers->push_back(
        idInfo(activeStack().getToken().value, 0, "", NULL));
    decStatement->identifiers->push_back(
        idInfo(activeStack().getToken().value, 0, "", NULL));
  ...
  while (true) {
    if (activeStack().getToken().value == "noblock") {
      ...
      if (decStatement->recurrent) {
        displayError(fName, activeStack().getToken().lineNumber,
                     activeStack().getToken().columnNumber,
                     "nodist variable can not be recurrent !");
      }

      ////displayWarning(fName,(tokens->at(activeStack().getIndex())).lineNumber,(tokens->at(activeStack().getIndex())).columnNumber,
      ///"Warning, nodist variable");
      distributed = false;
  ...
  if (peekToken("(")) {
    ...
    // decStatement->value = parseOpStatement(*working_tokens_index, to-1,
    // decStatement->type, 0, decStatement);
    decStatement->value =
        parseOpStatement(0, to - 1, decStatement->type, 0, decStatement);
    ...
    if (peekToken("{")) {
      ...
      while (activeStack().popToken().value.size()) {
        if (activeStack().getToken().value == "(") {
          ...
          if (plevel == 0) {
            ...
            if (0 != from) {
              // decStatement->values.push_back(parseOpStatement(from,
              // *working_tokens_index - 1, "-2", 0, decStatement));
              decStatement->values.push_back(parseOpStatement(
                  from, -1, decStatement->type, 0, decStatement));
            }
            break;
          }
        } else if (activeStack().getToken().value == ",") {
      ...
      if (decStatement->values.size() > (uint32_t)decStatement->size &&
          decStatement->size != 0) {
        displayError(fName, activeStack().getToken().lineNumber,
                     activeStack().getToken().columnNumber,
      ...
      for (uint32_t k = 0; k < decStatement->identifiers->size(); k++) {
        for (uint32_t i = 0; i < decStatement->values.size(); i++) {
          std::shared_ptr<termStatement> statement(new termStatement(
              decStatement->identifiers->at(k).name, decStatement->type));
          statement->scope = scope;

          SS << i;
          std::shared_ptr<termStatement> arrayIndex(
              new termStatement(SS.str(), decStatement->type));
          ...
          if (!decStatement->global) {
            idInfo id(decStatement->identifiers->at(k).name,
                      decStatement->scope, decStatement->type, arrayIndex);
            id.distributedScope = distributedScope;
            id.array = false;
      ...
      // decStatement->value = parseOpStatement(0, to-1, decStatement->type, 0,
      // decStatement); reachToken(terminal, true, true, true, false, false);
      // decStatement->value = parseOpStatement(0, to-1, decStatement->type, 0,
      // decStatement);

      decStatement->value =
          parseStatement("expression-list", &Parser::parseExpressionStatement);
      ...
      if (decStatement->value->type != decStatement->type) {
        if (!DM14::types::hasTypeValue(decStatement->type,
                                       decStatement->value->type)) {
          // error on type
          displayError(fName, -1, 0, "wrong value type for declaration value",
                       false);
        }
      }
      ...
      if (decStatement->global) {
        ...
        if (decStatement->splitStatements->size()) {
          decStatement->splitStatements->at(0)->distStatements =
              decStatement->distStatements;
  ...
  for (uint32_t i = 0; i < tempIdentifiers->size(); i++) {
    ...
    idInfo idinfo(tempIdentifiers->at(i).name, decStatement->scope,
                  decStatement->type, arrayIndex);
    ...
    for (uint32_t i = 0; i < datatypes.size(); i++) {
      if (datatypes.at(i).typeID == idinfo.type && datatypes.at(i).classType) {
        for (uint32_t k = 0; k < datatypes.at(i).memberVariables.size(); k++) {
          idInfo id = idInfo(
              datatypes.at(i).memberVariables.at(k).name, decStatement->scope,
              datatypes.at(i).memberVariables.at(k).returnType, NULL);
          /*FIX: bad ?! */
...
EBNF::callstack_t &Parser::activeStack() { return m_activeStack; }
...
parser_callback custom_callback)
{
        ...
        ebnfResult result = parseEBNF(working_tokens, starting_rule,
output_tokens);
        ...
        if(output == nullptr)
        {
                //displayError(prser->fName, errorToken.lineNumber,
errorToken.columnNumber,"Invalid Statement or grammar rule has no callback : " +
starting_rule + " at token : " + errorToken.value); displayError("prser->fName",
errorToken.lineNumber, errorToken.columnNumber,"Invalid Statement or grammar
rule has no callback : " + starting_rule + " at token : " + errorToken.value);
        }
