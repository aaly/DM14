...
bool scanner::isReady() { return iFile.is_open(); };

string scanner::getFileName() { return fName; };

scanner::scanner(const string &fileName) {
  fName = fileName;
  iFile.open(fileName.c_str(), ios::in);
  if (!iFile.is_open()) {
    displayError(fName, 0, 0,
                 "can't open source file for scanning : " + fileName);
  }
...
std::shared_ptr<Array<token>> scanner::getTokens() {
  if ((tokens->size() == 0) || (tokens == nullptr)) {
    return nullptr;
  }
  return tokens;
};
...
char scanner::getNextChar() {
  if (!iFile.is_open() || iFile.eof()) {
    displayError(
        fName, line, column,
        " can't get next char , end of file ?"); // column : " +
                                                 // tmptoken.columnNumber);
  }
...
int scanner::unGetChar() {
  if (!iFile.is_open() || iFile.eof()) {
    displayError(
        fName, line, column,
        " can't go back :( , end of file ?"); // column : " +
                                              // tmptoken.columnNumber);
  }
...
int scanner::printTokens() {
  if (tokens->size() != 0) {
    for (int unsigned i = 0; i < tokens->size(); i++) {
      cout << "tokens[" << i << "] " << "[" << (tokens->at(i)).type << "] "
           << "[" << (tokens->at(i)).lineNumber << "] " << "= "
           << (tokens->at(i)).value << endl;
    }
  } else {
    return (1);
  }
...
int scanner::addNumber() {
  ...
  if (ch == '.') {
    displayWarning(fName, line, column,
                   "Immediate float does not begin with 0 !");
    tmptoken.value += "0.";
    tmptoken.type = "float";
    ch = getNextChar();
  }
...
int scanner::addKeyword() { return 0; };
...
int scanner::addLetter() {
  tmptoken.value += ch;
  if (!(DM14::types::isLetter(iFile.peek()) ||
        DM14::types::isNumber(iFile.peek()))) {
...
int scanner::scan() {
  ...
  while (!iFile.eof() && (ch = getNextChar())) {
    ...
    } else if (ch == '"') // a double qutation , which means a string
    {
      ...
      addChar();
    } else if (DM14::types::isLetter(
                   ch)) // a letter , which means an identifier or a keyword !
    {
    ...
    // ch == '-' && ( DM14::types::isNumber(iFile.peek()) || iFile.peek() == '.'
    ...
    } else // we got a comment \o/
    {
      ...
      if (!read) {
        if (addOperator()) {
          // displayError(fName, line,0," couldn't recognize token :" + ch,
          // false);
        }
        tmptoken.value.clear();
      } else {
...
int scanner::setLongComment(const string &deliStart, const string &deliEnd) {
  ...
  if (!deliStart.size() || !deliEnd.size()) {
    displayError(
        fName, line, column,
        "Long comment start delimeter and end delimeter must have a size",
        false);
  }
...
int scanner::reachToken(const int &from, const string &Char,
                        const bool &sameLine, const bool &reportError,
  ...
  while (iterator++) {
    if (sameLine) {
      if (tokens->at(iterator).lineNumber != line) {
        if (reportError) {
          displayError(fName, (tokens->at(iterator)).lineNumber,
                       (tokens->at(iterator)).columnNumber, "Missing " + Char);
        } else {
          return -1;
        }
      }
    ...
    if ((beforeEOF && tokens->at(iterator).value == ";") ||
        tokens->at(iterator).value == Char) {
      return iterator;
    }
