...
// typedef Array<std::pair<std::string, std::pair<parser_callback,
// Array<token>>>> callstack_t;

class callstack_t {
public:
  callstack_t() { nested_stacks = make_shared<Array<callstack_t>>(); }

  std::shared_ptr<Array<callstack_t>> nestedStacks() { return nested_stacks; }
  ...
  void Print(uint32_t indent = 0) {
    ...
    cerr << "Rule : " << rule << " nested_stacks size:" << nested_stacks->size()
         << " tokens:[";
  ...
  int32_t nextIndex() {
    if (tokens->size() > 0 && index < tokens->size()) {
      ...
      displayError("prser>fName", (tokens->at(getIndex())).lineNumber,
                   (tokens->at(getIndex())).columnNumber, "Unexpected EOF");
    }
    return index;
  };
  ...
  token getToken() { return current_token; }
  ...
  std::shared_ptr<Array<token>> Tokens() { return tokens; }
...
enum class ebnfResultType { SUCCESS = 0, FAILURE };
...
public:
  ...
  int32_t nextIndex() {
    if (working_tokens->size() > 0 && index < working_tokens->size()) {
      index++;
    } else {
      displayError("prser>fName", (tokens->at(getIndex())).lineNumber,
                   (tokens->at(getIndex())).columnNumber, "Unexpected EOF");
      ...
      for (uint32_t i = 0; i < grammar[start_map_index].size(); i++) {
        ...
        for (uint32_t k = 0; k < current_rule.tokens.size(); k++) {
          ...
          if (ebnf_token.tokenType == DATATYPE_TOKEN) {
            ...
            cerr << " ERROR : unknown grammar token type : "
                 << typeid(ebnf_token.tokenType).name() << endl;
          }
          ...
          if (current_rule.type == GRAMMAR_TOKEN_OR) {
            ...
            //	cerr << string(EBNF_level, ' ') << " ERROR : unknown grammar
            //rule type" << endl;
          }
        }
        if (current_rule.tokens.size() > 1) {
  ...
  int32_t advance_EBNFindex(uint16_t steps = 1) {
    for (uint16_t i = 0; i < steps; i++) {
      if (*input_tokens_index < input_tokens->size()) {
        // cerr << "Pushing :  " << input_tokens->at(*input_tokens_index).value
        // << endl;
  ...
  int32_t deadvance_EBNFindex(uint16_t steps = 1) {
    for (uint16_t i = 0; i < steps; i++) {
      if (input_tokens->size()) {
        // cerr << "Removing :  " <<
        // working_tokens->at(working_tokens->size()-1).value << endl;
        removeToken();
        --*input_tokens_index;
      } else {
  ...
  std::shared_ptr<Statement> parseStatement(std::shared_ptr<Statement> output,
    ...
    if (output == nullptr) {
      ...
      // errorToken.columnNumber,"Invalid Statement or grammar rule has no
      // callback : " + starting_rule + " at token : " + errorToken.value);
      displayError("prser->fName", errorToken.lineNumber,
                   errorToken.columnNumber,
                   "Invalid Statement or grammar rule has no callback : " +
                       starting_rule + " at token : " + errorToken.value);
    }
...
private:
  /**
   * @details This function takes a set of input tokens, and a start map key,
   * and a pointer to the output tokens vector
   */
  ebnfResult parseEBNF(std::shared_ptr<Array<token>> input_tokens,
                       std::string start_map_index,
                       std::shared_ptr<Array<token>> output_tokens,
                       parser_callback callback = nullptr) {
    // cerr << "inside parseEBF : " << start_map_index << " :" <<
    // *input_tokens_index << endl;
    if (!grammar[start_map_index].size()) {
      cerr << "unknown grammar map index : " << start_map_index << endl
           << flush;
      exit(1);
    }
    ...
    cerr << endl
         << std::string(EBNF_level * 2, ' ')
         << ".LOOPING KEY : " << start_map_index << " : " << *input_tokens_index
         << endl
         << flush;
#endif
    ...
    if (groupByRules[start_map_index] &&
        rule_groups_depth < max_rule_groups_depth) {
      // cerr << ">>> entering local group :" << start_map_index << " [" <<
      // rule_groups_depth+1 << "]" << endl;
      rule_groups_depth++;
    ...
    /// loop over std::vector<grammar_token_array_t>
    for (uint32_t i = 0; i < grammar[start_map_index].size() &&
                         continue_searching_rules == true;
         i++) {
      result.status = ebnfResultType::SUCCESS;

#if PARSER_EBNF_SHOW_TRACE == 1
      cerr << string(EBNF_level * 2, ' ') << "..LOOPING RULE : " << i << " : "
           << *input_tokens_index << endl
           << flush;
#endif
      ...
      for (uint32_t k = 0;
           k < current_rule.tokens.size() && continue_searching_rules == true;
           k++) {
        EBNF_token_t ebnf_token = current_rule.tokens.at(k);
        int32_t token_old_index =
            *input_tokens_index; /// used for ONLY_ONE rules
        /** process the invidual token first */
        ebnfResult previous_result = result;
        result.status = ebnfResultType::FAILURE; /// initial value assumes error
                                                 /// , prove otherwise
        ...
        if (ebnf_token.tokenType == DATATYPE_TOKEN) {
          if (input_tokens->at(*input_tokens_index).type ==
              ebnf_token.expansion) {
            result.status = ebnfResultType::SUCCESS;
          }
        } else if (ebnf_token.tokenType == TERMINAL_TOKEN) {
          if (input_tokens->at(*input_tokens_index).value ==
              ebnf_token.expansion) {
            result.status = ebnfResultType::SUCCESS;
          }
        } else if (ebnf_token.tokenType == REGEX_TOKEN) {
          if (!DM14::types::isKeyword(
                  input_tokens->at(*input_tokens_index).value) ||
              ebnf_token.expansion == ".*") {
            std::regex self_regex(ebnf_token.expansion,
                                  std::regex_constants::ECMAScript);
            if (std::regex_match(input_tokens->at(*input_tokens_index).value,
                                 self_regex)) {
              result.status = ebnfResultType::SUCCESS;
            }
          }
        } else if (ebnf_token.tokenType == EXPANSION_TOKEN) {
          auto expansion_result = parseEBNF(input_tokens, ebnf_token.expansion,
                                            output_tokens, ebnf_token.callback);
          ...
          /*auto expansion_result = parseEBNF(input_tokens,
          ebnf_token.expansion, output_tokens, ebnf_token.callback);
          ...
          if (DM14::types::isSingleOperator(
                  input_tokens->at(*input_tokens_index).value)) {
            result.status = ebnfResultType::SUCCESS;
          }
        } else if (ebnf_token.tokenType == CORE_OP_TOKEN) {
          if (DM14::types::isCoreOperator(
                  input_tokens->at(*input_tokens_index).value)) {
            result.status = ebnfResultType::SUCCESS;
          }
        } else if (ebnf_token.tokenType == IMMEDIATE_TOKEN) {
          ...
          if (DM14::types::isbinOperator(
                  input_tokens->at(*input_tokens_index).value)) {
            result.status = ebnfResultType::SUCCESS;
          }
        } else if (ebnf_token.tokenType == OP_TOKEN) {
          if (DM14::types::isOperator(
                  input_tokens->at(*input_tokens_index).value) &&
              input_tokens->at(*input_tokens_index).value ==
                  ebnf_token.expansion) {
            result.status = ebnfResultType::SUCCESS;
        ...
        if (result.status == ebnfResultType::SUCCESS) {
          if (old_successful_depth < depth) {
            ...
            // std::cerr << "Update success map : " << old_depth.start_map_index
            // << "->" << depth.start_map_index  << std::endl;
          }

#if PARSER_EBNF_SHOW_TRACE == 1
          cerr << string(EBNF_level * 2, ' ')
               << "...PROCESSING TOKEN : " << ebnf_token.expansion
               << " => success " << endl
               << flush;
#endif
          ...
          if (old_depth < depth) {
            old_depth = depth;
            // std::cerr << "Update fail map : " << old_depth.start_map_index <<
            // "->" << depth.start_map_index  << std::endl;
          }
#if PARSER_EBNF_SHOW_TRACE == 1
          cerr << string(EBNF_level, ' ')
               << "...PROCESSING TOKEN : " << ebnf_token.expansion
               << " => fail " << endl
               << flush;
#endif
        ...
        if (*input_tokens_index >=
            input_tokens->size()) // break the zero or more cycle
        {
        ...
        if (current_rule.type == GRAMMAR_TOKEN_OR) {
          ...
          if (result.status != ebnfResultType::SUCCESS) {
#if PARSER_EBNF_SHOW_TRACE == 1
            cerr << string(EBNF_level * 4, ' ')
                 << "ERROR : tokens do not(follow order/exist) in map : "
                 << start_map_index << " rule : " << i << endl;
#endif
            // continue_searching_rules = false;
            break;
          } else {
...
#if PARSER_EBNF_SHOW_TRACE == 1
          cerr << string(EBNF_level * 4, ' ')
               << "ERROR : unknown grammar rule type" << endl;
#endif
        }
      }
    ...
    if (result.status == ebnfResultType::SUCCESS &&
      ...
      if (working_tokens->size() - current_working_tokens_size == 0 &&
        ...
        result.stack.rule = start_map_index;
        result.stack.setTokens(working_tokens->cut(current_working_tokens_size,
                                                   working_tokens->size()));
      }
    }
  ...
  std::map<std::string, bool> groupByRules; /** list of the groups to be used in
                                               the for the parseEBNF fuction */
  int32_t EBNF_level = -1;                  /** current EBNF level */

  int32_t token_index = -1; /** internal token index */
  Parser *prser =
      nullptr; /** internal parser pointer, used for the callbacks */
  parser_depth old_successful_depth; /** lat successfil EBND parsed statement */
  callstack_t callStack;             /** */
  uint32_t rule_groups_depth =
      0; /** counter of the current ebnf group level inside parseEBNF */
  uint32_t max_rule_groups_depth =
      2; /** maximum level of the ebnf groups depth */
  int32_t *input_tokens_index = nullptr;
  std::shared_ptr<Array<token>> tokens_stack =
      std::shared_ptr<Array<token>>(new Array<token>());
