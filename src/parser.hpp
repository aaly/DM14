...
class includePath {
public:
  enum class sourceFileType { FILE_DM14 = 1, FILE_C, LIBRARY };
  ...
  bool operator==(const includePath &other) const {
    if (package == other.package && library == other.library) {
      return true;
    }
    return false;
  }
...
public:
  mapcode();
  mapcode(const string &, std::shared_ptr<Array<ast_function>> const,
          const Array<includePath> &, const bool &, const int &, const int &);
  ~mapcode();
  string getFileName();
...
public:
  ...
  parseFunctionCallInternal(bool, const string &returnType = "",
                            const string &classID = "");
  // Statement*
  // parseConditionalExpression(Statement* stmt);
  ...
  std::shared_ptr<Statement>
  parseOpStatement(int32_t, int32_t, const string &, const int32_t &,
                   std::shared_ptr<Statement>,
                   std::shared_ptr<idInfo> parent = nullptr,
  ...
  // helper functions
  int32_t reachToken(const string &, const bool &, const bool &, const bool &,
  ...
  std::shared_ptr<Statement>
  findTreeNode(std::shared_ptr<Statement> opStatement, int32_t StatementType);
  int32_t searchVariables(std::shared_ptr<Statement> opStatement,
                          int32_t depencyType, string op = "");
  ...
  // int32_t						findFUNCInfo(const
  // string&, const int32_t&);
  funcInfo getFunc(const string &, const string &classID = "");
  funcInfo getFunc(const string &, Array<string> *, const string &returnType,
                   const string &classID);
...
private:
  ...
  DM14::EBNF::EBNF ebnf;
  Array<EBNF::callstack_t>
      stackList; /// the stack nodes produced the the EBNF Parser
  ...
  std::shared_ptr<Array<std::shared_ptr<distributingVariablesStatement>>>
      dvList;
  std::shared_ptr<Array<std::shared_ptr<Statement>>> linkLibs;

  long parseCClass(std::shared_ptr<scanner> scner, uint32_t start,
                   const Array<string> &templateName);
  funcInfo parseCFunction(std::shared_ptr<scanner> scner, uint32_t start,
  ...
  Statement* parseStatementInternal(Statement* output, const std::string
  starting_rule = "", EBNF::parser_callback custom_callback = nullptr);*/
};
