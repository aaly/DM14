...
int nspill(const string &); // for new line ( if parameter is empty , then just
                            // print new line ) , same as above but with endl
int spill(const int &);
...
// we could use template ofcourse but we will have to add that to the parser ,
// meh :D
int get(int &);
int get(char &);
int get(double &);
