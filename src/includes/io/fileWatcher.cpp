...
fileWatcher::fileWatcher() { fd = inotify_init1(IN_NONBLOCK); };
...
bool fileWatcher::isReady() { return fd > 0 ? true : false; };
...
bool fileWatcher::hasChanged(int desc) {
  ...
  for (unsigned int i = 0; i < eventsQueue.size(); i++) {
    if (eventsQueue.at(i)->wd == desc) {
      // if ( eventsQueue.at(i).mask & IN_CREATE || eventsQueue.at(i).mask &
      // IN_DELETE || eventsQueue.at(i).mask & IN_MODIFY )
