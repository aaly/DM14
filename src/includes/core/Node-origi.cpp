...
int Node::requestObject(unsigned int var, int index, string nodeAddress) {
  ...
  nodeMessage R((int)SS.str().at(0), SS.str().substr(1, SS.str().size()),
                *node);
...
int Node::pollOnSocket(int socketFD,
                       short events = EPOLLIN | EPOLLPRI | EPOLLHUP | EPOLLERR |
                                      EPOLLMSG | EPOLLET | EPOLLWRBAND |
                                      EPOLLRDBAND | EPOLLRDNORM |
...
int Node::deSerialize(int var, int index) {
  ...
  if (type == "int") {
    ...
    *object = tmp;
    cerr << "FLOAR:"
         << *(string **)_DM14VARIABLESMAP.at(var).buffer->at(
                variable->buffer->size() - 1)
         << endl;
    cerr << "FLOAR:" << *(string *)_DM14VARIABLESMAP.at(var).buffer->at(0)
         << endl;
  } else if (type == "char") {
    char *tmp = new char;
