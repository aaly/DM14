...
public:
  LocalDataVectoryEntry()
      : array(false), stack(NULL), elements(NULL), recurrent(false),
        channel(false), init(false), ready(false) {
  ...
  LocalDataVectoryEntry(bool parray, bool pready, void *pvar, bool precurrent,
      ...
      : array(parray), stack(NULL), type(ptype), elements(NULL),
        recurrent(precurrent), channel(pchannel), init(false), ready(pready) {
    buffer = new Array<void *>;
  };
  bool array;
...
public:
  ...
  /*nodePeer(const nodePeer &np) {peer = np.peer; node = np.node; ready =
     np.ready; remainingMsgSize = np.remainingMsgSize; optimizations =
     np.optimizations; runningFlags = np.runningFlags; status = np.status; Port
     = np.Port; IP = np.IP; nodeID = np.nodeID;};*/
  ~nodePeer();
...
public:
  ...
  template <class T>
  void addVectorArrayData(uint32_t var, long index, bool parray, bool pready,
                          void *pvar, bool precurrent, bool pchannel,
                          const string &ptype);
  template <class T>
  void addVectorClassData(uint32_t var, long index, bool parray, bool pready,
                          void *pvar, bool precurrent, bool pchannel,
                          const string &ptype);
  template <class T>
  void addVectorData(uint32_t var, long index, bool parray, bool pready,
                     void *pvar, bool precurrent, bool pchannel,
                     const string &ptype);

  template <class T>
  void need(uint32_t var, int index, bool pointer, int node, T *variable,
            bool block, string nodeID = "");
  template <class T>
  void done(uint32_t var, int index, void *pointer, string nodeID = "");
...
template <class T>
void Node::need(uint32_t var, int index, bool pointer, int node, T *variable,
                bool block, string nodeID) {
...
template <class T>
void Node::done(uint32_t var, int index, void *pointer, string nodeID) {
...
template <class T>
void Node::addVectorArrayData(uint32_t var, long index, bool parray,
                              bool pready, void *pvar, bool precurrent,
                              bool pchannel, const string &ptype) {
  if (parray) {
...
template <class T>
void Node::addVectorClassData(uint32_t var, long index, bool parray,
                              bool pready, void *pvar, bool precurrent,
                              bool pchannel, const string &ptype) {
...
template <class T>
void Node::addVectorData(uint32_t var, long index, bool parray, bool pready,
                         void *pvar, bool precurrent, bool pchannel,
                         const string &ptype) {
  if (index > -1) {
