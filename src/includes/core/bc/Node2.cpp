...
void *Node::listener(void *par) // thread;
{
  ...
  while (Serve) {
    ...
    for (unsigned int i = 0, s = clients.size(); i < s; i++) {
      ...
      if (msg.size() > 0) {
        // cout << "got message 1 : " << msg <<  " -> " << (int)msg.at(0) <<
        // endl << flush;
    ...
    for (unsigned int i = 0, s = nodes.size(); i < s; i++) {
      for (unsigned int l = 0; l < 5; l++) {
        ...
        if (msg.size() > 0) {
          // cout << "got message : " << msg <<  " -> " << (int)msg.at(0) <<
          // endl << flush;
...
int Node::pushRequestMessage(const string &msg, nodePeer node,
  ...
  // if(R.getNode().ready)
  if (R.getNode().ready || (R.getType() == nodeMessage::R_JOIN ||
...
int Node::handleRequest(nodeMessage &R) {
  cout << "Handling Request:" << nodeMessage::typeName[R.getType()] << " "
       << R.getValue() << " " << endl
       << flush;

  if (R.getType() == nodeMessage::ADD_NODE ||
      R.getType() == nodeMessage::ADD_NODE_BOOTSTRAP) {
    // cout << "Handling Request: ADD node : " << R.getType() << " " <<
  ...
  if (R.getType() == nodeMessage::R_OBJECT_FROM_NODE) {
    ...
    for (unsigned int i = 0; i < nodes.size(); i++) {
      ...
      if (node == -2 || nodes.at(i).node == node) {
        ...
        if (nodes.at(i).status == nodePeer::FOREVERALONE ||
            !nodes.at(i).ready) {
          cerr << "NEED 1 : " << R.getValue() << ":" << nodes.at(i).nodeID
               << endl
               << flush;
          sent = -1;
          continue;
        ...
        if (node == -2) {
          if (variable->registerWriter(nodes.at(i).nodeID) == 1) {
            //??
            cerr << "NEED 2 : " << R.getValue() << ":" << nodes.at(i).nodeID
                 << endl
                 << flush;
    ...
    // cout << "Handling Request:" << R.getType() << " " <<  R.getValue() << " "
