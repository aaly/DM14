...
{
	public:
		//LocalDataVectoryEntry():array(false),ready(false),var(NULL),elements(new Array<LocalDataVectoryEntry>()){};
		//LocalDataVectoryEntry():array(false),ready(false),var(NULL), elements(NULL), recurrent(false), channel(false), init(false), stack(false), lockBit(0){ buffer = new Array<void*>;};
		//LocalDataVectoryEntry():array(false),ready(false),var(NULL), elements(NULL), recurrent(false), channel(false), init(false), stack(false), lockBit(0){ buffer = new Array<void*>;};
		LocalDataVectoryEntry():array(false),ready(false),var(NULL), elements(NULL), recurrent(false), channel(false), init(false), stack(false) {buffer = new Array<void*>;};
		LocalDataVectoryEntry(bool parray, bool pready, void* pvar, bool precurrent, bool pchannel, const string& ptype):array(parray),ready(pready),var(pvar),type(ptype), elements(NULL), recurrent(precurrent), channel(pchannel), init(false), stack(false){buffer = new Array<void*>;};
		//LocalDataVectoryEntry(bool parray, bool pready, void* pvar, bool precurrent, bool pchannel, const string& ptype):array(parray),ready(pready),var(pvar),type(ptype), elements(NULL), recurrent(precurrent), channel(pchannel), init(false), stack(false), lockBit(0){buffer = new Array<void*>;};
		bool	array;
		//bool	element;
		bool	ready;
		void*	var;
...
{
	public:
		nodeMessage(const int& type, const string& msg, nodePeer& node) : m_type(type), m_msg(msg), m_node(node){};
		nodeMessage();
		nodePeer&	getNode();
		int const	getType();
		string&		getValue();
...
class Node
{
	public:
		...
		//template<class T> T& need(int var, string type, unsigned int node, T& variable);
		//void need(int var, string type, unsigned int node, void* variable);
		//Array<void*> 	__M14VPOINTERS;
		//Array<bool>		 __M14VSTATUSES;
	...
	private:
		...
		int		pushRequestMessage(const string& msg, nodePeer node = nodePeer(), const bool& insider = false);
...
{
	//* should we loop through requests and sent them ? <<<< just tell them it is ready
	//* or let the clients loop and get it ? <<<< better because they are waiting for it anyway !
	...
	if(!variable->stack || variable->channel || variable->recurrent || index != -1)
	{
		...
		*(T*)variable->var = *(T*) pointer;
		...
		if(!nodeID.size())
		{
			...
			{
				//displayError("Sending Channel obect to " + variable->readersParty.at(i));
...
template<class T> void Node::addVectorArrayData(int var, long index, bool parray, bool pready, void* pvar, bool precurrent, bool pchannel, const string& ptype)
{
	if(parray)
	{
		////cout << "adding: " << var << "-" << index << "-" << ptype << "-" << endl;
