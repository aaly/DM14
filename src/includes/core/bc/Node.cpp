...
Node::Node()
{
	...
	pollOnSocket(pollTrigger[1], POLLIN);
	pollOnSocket(Self.getSocketDescriptor(), POLLIN|POLLOUT);
...
int Node::pollOnSocket(int socketFD, short events = POLLIN | POLLPRI | POLLHUP | POLLNVAL | POLLERR | POLLREMOVE | POLLMSG)
{
...
{	
	...
	{	
		...
		pollOnSocket(parent.peer->getSocketDescriptor(), POLLIN);
		//nodes.unlock();
...
void* Node::listener(void* par) // thread;
{
	...
	{
		processMessages();
		//int rc = poll(monitoredDescriptors, currentMonitoredDescriptorsCount, checkTicks);
		int rc = poll(monitoredDescriptors, currentMonitoredDescriptorsCount, -1);
		...
		int current_size = currentMonitoredDescriptorsCount;
		for (int i = 0; i < current_size; i++)
		{
			//cerr << "I :" << i << " E:" << monitoredDescriptors[i].revents << endl;
			if(monitoredDescriptors[i].revents == 0)
			{
				continue;
			}
			else if(monitoredDescriptors[i].revents&POLLHUP ||
					monitoredDescriptors[i].revents&POLLNVAL ||
					monitoredDescriptors[i].revents&POLLERR ||
					monitoredDescriptors[i].revents&POLLREMOVE ||
					monitoredDescriptors[i].revents&POLLMSG)
			{
				pollOffSocket(monitoredDescriptors[i].fd);
				removeNode(*findNodeByFd(monitoredDescriptors[i].fd));
				continue;
			}
			else
			{
				//cerr << "EVENT :" << 	monitoredDescriptors[i].revents << endl;
			}
			
			
			//1. accept clients if exists; should be while and not if ?
			if (monitoredDescriptors[i].fd == Self.getSocketDescriptor())// && (monitoredDescriptors[i].revents & POLLIN))
			{				
			...
			else if (monitoredDescriptors[i].fd == pollTrigger[1])
			{
				char buff;
			...
			{
				nodePeer* peer = findNodeByFd(monitoredDescriptors[i].fd);
				if(peer)
				{
					//int count;
		...
		{
			string msg = nodes.at(i).peer->recvMessage();
			if ( msg.size() > 0 )
			{
...
{	
	...
	else if (R.getType() == nodeMessage::R_JOIN || R.getType() == nodeMessage::R_JOIN_BOOTSTRAP)
	{
		...
		/*for(unsigned int i =0; i < _DM14VARIABLESMAP.size(); i++)
		{
			...
			nodeMessage R2(nodeMessage::R_OBJECT_FROM_NODE, SS.str(), nodes.at(nodes.size()-1));
	...
	else if (R.getType() == nodeMessage::R_OBJECT || R.getType() == nodeMessage::R_CHANNELOBJECT)
	{
		...
		else
		{
			if(variable->channel || variable->recurrent)
			{
				//if(variable->registerReader(R.getNode().nodeID) == 0 || variable->channel || variable->recurrent)
				{
