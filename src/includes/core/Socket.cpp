...
struct addrinfo* fillAddr(const string &address, const string port, struct addrinfo* addr)
{
...
//	hints.ai_flags = AI_PASSIVE;     // fill in my IP for me
	...
	if((status = getaddrinfo(address.c_str(), port.c_str(), &hints, &addr)) != 0)
	{
	...
	{
		//*addr = *addr2;
		//cerr << "A:" << address << " P:" << port << ":" << addr->ai_addrlen << ":" << inet_ntoa((struct in_addr)addr->sin_addr) << endl;
...
int Socket::setLocalAddressAndPort(const string &localAddress, const string &localPort)
{
...
if(value > 0)
{
	...
	if ((rtn = ::recv(sockDesc, (raw_type *) &data, bSize, MSG_WAITALL)) < 0)
	{
		...
		//cerr << "recvv1 : " << recieved << " : " << recieved.size() << " : " << rtn << endl << flush;
		return "";
	}
	else if(rtn == 0)
	{
		//cerr << "recvv2 : " << recieved << " : " << recieved.size() << " : " << rtn << endl << flush;
		return "";
	}
	else
	{
	...
	//cerr << "recvv : " << recieved << " : " << recieved.size() << " : " << rtn << endl << flush;
...
TCPSocket::TCPSocket(const string &foreignAddress, const string& foreignPort)  throw(SocketException) : 
																				Socket(SOCK_STREAM, IPPROTO_TCP)
{
	setopts();
...
TCPSocket::TCPSocket(const string &localAddress, const string& localPort, int queueLen) 
...
MessageTCPSocket::MessageTCPSocket(const string& foreignAddress, const string& foreignPort)
...
MessageTCPSocket::MessageTCPSocket(const string &localAddress, const string& localPort, int queueLen) 
