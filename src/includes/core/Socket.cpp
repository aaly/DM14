...
struct addrinfo *fillAddr(const string &address, const string port,
  ...
  */
  ...
  if ((status = getaddrinfo(address.c_str(), port.c_str(), &hints, &addr)) !=
      0) {
    fprintf(stderr, "getaddrinfo error: %s\n", gai_strerror(status));
  }

  // struct sockaddr_in *addr2;
  // addr2 = (struct sockaddr_in *)addr->ai_addr;

  if (addr) {
    //*addr = *addr2;
    // cerr << "A:" << address << " P:" << port << ":" << addr->ai_addrlen <<
    // ":" << inet_ntoa((struct in_addr)addr->sin_addr) << endl;
...
int Socket::getSocketDescriptor() { return sockDesc; }

int Socket::Close() { return close(sockDesc); }

int Socket::Shutdown() { return shutdown(sockDesc, SHUT_RDWR); }
...
// int Socket::setLocalAddressAndPort(const string &localAddress, unsigned short
// localPort)
int Socket::setLocalAddressAndPort(const string &localAddress,
                                   const string &localPort) {
  // Get the address of the requested host
  struct addrinfo *localAddr = 0;
  localAddr = fillAddr(localAddress, localPort, localAddr);

  if (!localAddr) {
    cerr << "ERROR IN FILLADDR" << endl;
    return -1;
  }

  int res = ::bind(sockDesc, localAddr->ai_addr, localAddr->ai_addrlen);
  if (res < 0) {
    cerr << "ERROR IN BIND" << endl;
    cerr << localAddr->ai_addrlen << endl;
    cerr << localAddr->ai_addr->sa_data << endl;
...
int Socket::connect(const string &foreignAddress, const string &foreignPort) {
  // Get the address of the requested host

  struct addrinfo *destAddr = 0;

  destAddr = fillAddr(foreignAddress, foreignPort, destAddr);

  int result = -1;

  // Try to connect to the given port
  if (destAddr) {
    result = ::connect(sockDesc, destAddr->ai_addr, destAddr->ai_addrlen);
    if (EINVAL == errno) {
      cerr << "EINVAL" << endl;
    }
  }
...
int Socket::recv(void *buffer, int bufferLen) {
  int rtn;
  // if ((rtn = ::recv(sockDesc, (raw_type *) buffer, bufferLen, MSG_ERRQUEUE))
  // < 0)
  if ((rtn = ::recv(sockDesc, (raw_type *)buffer, bufferLen, MSG_NOSIGNAL)) <
      0) {
...
cout << "in buffer : " <<  value << endl;
  ...
  char data[bSize];
  string recieved;
  // ssize_t bytes_received =  ::recv(sockDesc, (raw_type *) &data, bSize,
  // MSG_PEEK);
  ...
  if ((rtn = ::recv(sockDesc, (raw_type *)&data, bSize, MSG_WAITALL)) < 0) {
    ...
    {
            cout << strerror(rtn) << endl;;
            cout <<"Socket error:" << "can not recieve:" << errno << endl <<
    flush;
            //exit(1);
            Error = 1;
    }*/
    // cerr << "recvv1 : " << recieved << " : " << recieved.size() << " : " <<
    // rtn << endl << flush;
    return "";
  } else if (rtn == 0) {
    // cerr << "recvv2 : " << recieved << " : " << recieved.size() << " : " <<
    // rtn << endl << flush;
    return "";
  } else {
    for (int i = 0; i < rtn; i++) {
  ...
  // cerr << "recvv : " << recieved << " : " << recieved.size() << " : " << rtn
  // << endl << flush;
...
// foreignPort)  throw(SocketException) : Socket(SOCK_STREAM, IPPROTO_TCP)
...
TCPSocket::TCPSocket(int newConnSD) : Socket(newConnSD) { setopts(); }

TCPSocket::TCPSocket(const string &localAddress, const string &localPort,
...
int TCPSocket::setopts() { return 0; }
...
MessageTCPSocket::MessageTCPSocket() throw(SocketException)
    : TCPSocket()
...
MessageTCPSocket::MessageTCPSocket(
    ...
    const string &foreignPort) throw(SocketException)
    : TCPSocket((const string &)foreignAddress, foreignPort)
...
MessageTCPSocket::MessageTCPSocket(const string &localAddress,
                                   const string &localPort,
                                   int queueLen) throw(SocketException)
    : TCPSocket(localAddress, localPort, queueLen) {
  setopts();
  /*setLocalAddressAndPort(localAddress, localPort);
  listen(queueLen);*/
}
...
int MessageTCPSocket::sendMessage(const string &msg) {
  ...
  if (msg.size() > 0) {
    ...
    if (bufferSend) {
      sendBuffer = sendBuffer + size + msg;
      ret = -2;
    } else {
      ret = Socket::send((void *)((size + msg).data()), (size + msg).size());
    }
  } else {
...
bool Socket::enableNagle(bool cond) {
  int state = (int)cond;
  int result =
      setsockopt(sockDesc, IPPROTO_TCP, TCP_NODELAY, &state, sizeof(state));
...
bool Socket::isReadyToRead(int _socketHandle,
                           const long &_lWaitTimeMicroseconds) {
  int iSelectReturn =
      0; // Number of sockets meeting the criteria given to select()
  timeval timeToWait;
  ...
  fd_set readSetOfSockets; // Bitset representing the socket we want to read
                           // 32-bit mask representing 0-31 descriptors where
                           // each bit reflects the socket descriptor based on
                           // its bit position.
  ...
  // iSelectReturn -1: ERROR, 0: no data, >0: Number of descriptors found which
  // pass test given to select()
  ...
  // Got here because iSelectReturn > 0 thus data available on at least one
  // descriptor Is our socket in the return list of readable sockets
