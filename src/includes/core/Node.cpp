...
int Node::pollOnSocket(int socketFD, uint32_t events = EPOLLIN | EPOLLHUP | EPOLLERR)
{
...
int Node::addNode(const string& address, const string& port, bool bootstrap) // to be called by main
{
	nodePeer parent;
	parent.node = 0;
	...
	{
		cerr << "Failed to connect to parent " << address << ":" << port << endl << flush;
		exit(1);
	}
	return 0;
};
...
int Node::setServer(const string& address, const string& port, const int& clients) // to be called by main
{
	//serverInit = true;
...
{	
	cerr << "Handling Requesttt:" << Node::messageTypeName[R.getType()] << " " <<  R.getMessage() << " " << endl << flush;
	
	if (R.getType() == Node::ADD_NODE || R.getType() == Node::ADD_NODE_BOOTSTRAP)
	{
		nodeMessage reply;
	...
	else if (R.getType() == Node::R_JOIN || R.getType() == Node::R_JOIN_BOOTSTRAP)
	{
		cout << "Handling Request Request Join :" << R.getType() << " " <<  R.getMessage() << " " << endl << flush;
	...
	{
		//cout << "Handling ACCEPTED :" << R.getType() << " " <<  R.getMessage() << " " << endl << flush;
		//findNode(R.getNode().nodeID)->ready = true;
		nodes.at(R.getNodeIndex()).ready = true;
		nodes.at(R.getNodeIndex()).node = strToNumber(R.getMessage().substr(0, R.getMessage().find(';', 0)));
	...
	else if (R.getType() == Node::R_OBJECT || R.getType() == Node::R_CHANNELOBJECT)
	{
	...
	else if (R.getType() == Node::N_R_OBJECTNOTFOUND)
	{
		...
		index = strToNumber(R.getMessage().substr(R.getMessage().find(';')+1, R.getMessage().size()));*/
	}
	else if (R.getType() == Node::N_R_OBJECTVALUE)
	{
		//cout << "got item " << R.getValue() << " from :" << R.getNode().nodeID << endl << flush;
		//cout << "got item " << R.getValue() << " from :" << nodes.at(R.getNodeIndex()).nodeID << endl << flush;
		int var = 0;
		int index = -1;
		unsigned int size = 0;
		LocalDataVectoryEntry* variable = NULL;
