...
class objectDistance
{
	...
	private:
		float determineFocalLength(float realWidth, float pixlesWidth, float distance, float matWidth = 0);
		...
		openCvMat getImage();
		int findOneItemDistance(pointsArray& contours, openCvMat& frame, int minimumArea);
