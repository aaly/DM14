...
bool rectOverlap(int Ax, int Ay, int Awidth, int Aheight,
				 int Bx, int By, int Bwidth, int Bheight)
{
    bool xOverlap = valueInRange(Ax, Bx, Bx + Bwidth) ||
                    valueInRange(Bx, Ax, Ax + Awidth);

    bool yOverlap = valueInRange(Ay, By, By + Bheight) ||
...
float objectDistance::determineFocalLength(float realWidth, float pixlesWidth, float distance, float matWidth = 0)
{
	cerr << "Calcualte with : " << pixlesWidth << ":" << distance << ":" << realWidth << endl;
...
{	
	...
	//float distance = (pixlesToCM(rect.br().y-rect.tl().y)*focalLength)/(rect.br().y-rect.tl().y);
...
bool objectDistance::initImageSource(string srcPath)
{
	...
	else if (imageSource == VIDEOSRC || imageSource == CAMSRC)
	{
		...
		if(!videoCapture.isOpened())
		{
			if(imageSource == VIDEOSRC)
			{
				cerr << "[Error]: Could not open video file : " << srcPath << endl << flush;
			}
			else
			{
...
bool objectDistance::newDataAvailable()
{
	...
	else if((imageSource == VIDEOSRC) && readCounter > videoCapture.get(CV_CAP_PROP_FRAME_COUNT))
	{
		return false;
	}
...
openCvMat objectDistance::getImage()
{
	if (!initDone)
	{
		cerr << "[Error] : tried to get a mage before initilizating the source" << endl << flush;
	}
	...
	else if (imageSource == VIDEOSRC || imageSource == CAMSRC)
	{
		...
		if(videoCapture.isOpened() && videoCapture.grab() &&videoCapture.retrieve(currentImage))
		{
			readCounter++;
			return currentImage;
		}
...
{
    cout << "\nthis application is used to Detect red led and determine the distance \n"
            "Usage:\n"
...
{  
   ...
   inRange(imgHSV,  lowHSV,
					highHSV,
					imgHSV); //Threshold the image

	//morphological opening (removes small objects from the foreground) Morphological opening
	...
	//morphological closing (removes small holes from the foreground) Morphological closing
	dilate( imgHSV, imgHSV, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) ); 
	erode(imgHSV, imgHSV, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) );
...
/*bool objectDistance::circleInsideCircle(Point2f center1, float radius1, Point2f center2, float radius2)
{
...
int objectDistance::findOneItemDistance(pointsArray& contours, openCvMat& imageThreshMat, int minimumArea)
{	
	////std::vector<std::vector<Point> > contours;
	//blur( imageThreshMat, imageThreshMat, Size(3,3) );
	//Canny( imageThreshMat, imageThreshMat, 100, 50*3, 3);
	
	////findContours( imageThreshMat, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE );
	...
	{
		approxPolyDP(Mat(contours[i]), contor_poly[i], 0.01*arcLength(contours[i],true), true);
		//circle( imageThreshMat, finalCenter[i], (int)finalRadius[i], cv::Scalar(255, 0, 0), 2, 8, 0 );
		if(contor_poly[i].size() > 4)
		{
			final_contours.push_back(contours[i]);
			minEnclosingCircle( (Mat)contor_poly[i], center[i], radius[i] );
			circle( imageThreshMat, center[i], (int)radius[i], cv::Scalar(255, 0, 0), 2, 8, 0 );
		}
	}
	...
	for (int i = 1000000; i < center.size(); i++) 
	{
		...
		for (int k = 0; k < center.size(); k++) 
		{
			...
			int intersection = rectOverlap( topLeftX, topLeftY, radius[i]*2, radius[i]*2,
	...
	{
		//drawContours(frame, final_contours, idx, cv::Scalar(255, 0, 0), CV_FILLED);
		///drawContours(imageThreshMat, final_contours, i, cv::Scalar(255, 0, 0));
		circle( frame, finalCenter[i], (int)finalRadius[i], cv::Scalar(255, 0, 0), 2, 8, 0 );
		
		char readCounterString[20];
		snprintf(readCounterString, sizeof(readCounterString), "frame : %d", readCounter);
		putText(frame, readCounterString,  cvPoint(10,20), FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(200,200,250), 1, CV_AA);
		
		char objectDistanceString[100];
		snprintf(objectDistanceString, sizeof(objectDistanceString), "distance : %.f cm", calculateDistance(finalRadius[i]*2));
		putText(frame, objectDistanceString,  cvPoint(finalCenter[i].x, finalCenter[i].y+finalRadius[i]), FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(250,250,250), 1, CV_AA);
...
int objectDistance::drawBoundingRects(pointsArray& contours, openCvMat& frame)
{
	...
	for(int i =0; i< boundRect.size(); i++)
	{
		...
		if(a > 600)
		{
			...
			for( int j = 0; j < 4; j++ )
			{
				line( frame, rect_points[j], rect_points[(j+1)%4], Scalar(255,25*i,0), 1, 8 );
			}
			//rectangle(frame, boundRect[i].boundingRect().tl(), boundRect[i].boundingRect().br(), Scalar(255,255,0), 2, 8, 0);
			char objectDistanceString[100];
			snprintf(objectDistanceString, sizeof(objectDistanceString), "distance : %.f cm", calculateDistance(boundRect[i].boundingRect()));
			//cout << boundRect[i].boundingRect()
			//snprintf(objectDistanceString, sizeof(objectDistanceString), "distance : %d cm", calculateDistance(boundRect[i].boundingRect()));
			putText(frame, objectDistanceString,  cvPoint(boundRect[i].boundingRect().tl().x, boundRect[i].boundingRect().br().y+15), FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(250,250,250), 1, CV_AA);
	...
	if (index != -1)
	{
		...
		for( int j = 0; j < 4; j++ )
		{
			line( frame, rect_points[j], rect_points[(j+1)%4], Scalar(255,25*index,0), 1, 8 );
		}	
		//rectangle(frame, boundRect[i].boundingRect().tl(), boundRect[i].boundingRect().br(), Scalar(255,255,0), 2, 8, 0);
		char objectDistanceString[100];
		snprintf(objectDistanceString, sizeof(objectDistanceString), "distance : %.f cm", calculateDistance(boundRect[index].boundingRect()));
		//cout << boundRect[i].boundingRect()
		//snprintf(objectDistanceString, sizeof(objectDistanceString), "distance : %d cm", calculateDistance(boundRect[i].boundingRect()));
		putText(frame, objectDistanceString,  cvPoint(boundRect[index].boundingRect().tl().x, boundRect[index].boundingRect().br().y+15), FONT_HERSHEY_COMPLEX_SMALL, 0.8, cvScalar(250,250,250), 1, CV_AA);
	}	
	return 0;
}
...
string objectDistance::getSrcPath()
{
	return srcPath;
}
...
int objectDistance::Init()
{
	...
	{
		cerr << "[Help] : is the source path specified correctly ?" << endl << flush;
		//help();
		return -1;
	}
...
int objectDistance::getCurrentDistance()
{
	...
	//cv::inRange(imgHSV, cv::Scalar(165, 140, 120), cv::Scalar(180, 255, 255), lower_red_hue_range);
	cv::inRange(imgHSV, cv::Scalar(165, 120, 110), cv::Scalar(180, 255, 255), lower_red_hue_range);
	cv::inRange(imgHSV, cv::Scalar(0, 130, 130), cv::Scalar(8, 255, 255), upper_red_hue_range); // orange? !
	// Combine the above two images
	cv::addWeighted(lower_red_hue_range, 1.0, upper_red_hue_range, 1.0, 0.0, imgHSV);
	...
	cv::Mat structuringElement = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(80, 80));
	cv::morphologyEx( imageThreshMat, imageThreshMat, cv::MORPH_CLOSE, structuringElement );
	
	//erode(imageThreshMat, imageThreshMat, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) );
	//dilate(imageThreshMat, imageThreshMat, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) ); 
