...
public:
  ...
  // Array < map<string, map <int, map < int, void*> > >* > vars; // map (
  // identifier , map ( type, map ( operator, value) )
...
class idInfo {
public:
  idInfo(const string &, const int &, const string &,
         std::shared_ptr<Statement>);
  idInfo(const string &, const int &, const string &, const string &,
         std::shared_ptr<Statement>);
  idInfo();
  string name;
  int scope;
...
public:
  enum dvStatementType { MODS = 128, DEPS };
...
public:
  ...
  // int				type;				// type
  // , int float , etc.. int				op;
  // // operator like += or -= so on ... , we really need it ??
...
public:
  ...
  // std::shared_ptr<Array<std::shared_ptr<Statement>>>		right;	//
  // termStatements ?
...
public:
  ...
  std::shared_ptr<Array<std::shared_ptr<Statement>>>
      parameters; // Array of parameters with void , that can be anything
  int functionType;
};
...
public:
  ...
  // string step;						// the step
  // value
  std::shared_ptr<Array<std::shared_ptr<Statement>>> stepCondition;
  std::shared_ptr<Array<std::shared_ptr<Statement>>>
      body; // will point to the Statements in the Statements Array
};
...
// the starter value std::shared_ptr<Array<std::shared_ptr<Statement>>> body;
// will point to the Statements in the Statements Array
};*/
...
public:
  ...
  ~CASE();
  std::shared_ptr<Statement> condition;
  // std::shared_ptr<Array<std::shared_ptr<Statement>>>	cases;		// Array
  // of parameters with void , that can be anything
  // std::shared_ptr<Array<std::shared_ptr<Statement>>>	body;		// Array
  // of parameters with void , that can be anything
...
public:
  ...
  //  when user define a function , get its info into the info map and then we
  //  can search for its info later
